...
struct BVHBuffers
{
    ...
    BVHBuffers(size_t max_vertices) : capacity(max_vertices), initialized(false)
    {
                        ...
                        const T   kappa)
{
    ...
    vec3<T> offsets[5] = {vec3<T>(0, box_min_y, 0),
                ...
                const VertexAttribute<int>& region_label)
{
    ...
    size_t nodes_memory =
        bvh.numNodes * sizeof(typename cuBQL::BinaryBVH<T, 3>::Node);
    ...
    rx.for_each_vertex(DEVICE, [=] __device__(const VertexHandle& vh) mutable {
        ...
        auto query_lambda = [&](uint32_t prim_id) -> float {
            ...
            VertexHandle other_vh =
                ctx.template get_handle<VertexHandle>(prim_id);
    ...
    // RXMESH_INFO("  BVH memory: {:.2f} MB", total_bvh_memory / (1024.0f *
    // 1024.0f));
    ...
    problem.template add_interaction_term<Op::VV, true>(
        [=] __device__(
            ...
            if (d < dhat) {
                ...
                T avg_contact_area =
                    (contact_area(v0) + contact_area(v1)) * T(0.5);

                // Barrier energy: E = h^2 * A * dhat * 0.5 * kappa * (s - 1) *
                // log(s)
                E = h_sq * avg_contact_area * dhat * T(0.5) * kappa * (s - 1) *
                    log(s);
            }
    ...
    problem.template add_interaction_term<Op::VF, true>(
        [=] __device__(const auto& fh,
            ...
            if (d < dhat) {
                ...
                // Compute triangle area using cross product: Area = 0.5 *
                // ||(p1-p0) × (p2-p0)||
                ...
                // Barrier energy: E = h^2 * A * dhat * 0.5 * kappa * (s - 1) *
                // log(s)
                E = h_sq * triangle_area * dhat * T(0.5) * kappa * (s - 1) *
                    log(s);
            }
...
__global__ static void build_triangle_boxes_kernel(const Context context,
                ...
                const FaceAttribute<uint64_t>& face_vertices)
{
    ...
    build_triangle_boxes_kernel<blockThreads, T>
        <<<launch_box.blocks,
           ...
           launch_box.smem_bytes_dyn>>>(rx.get_context(), x, d_boxes);
    CUDA_ERROR(cudaDeviceSynchronize());
    ...
    // Note: this is a more broader/lenient query as we're not testing for exact
    // closeness here This is because right now there isn't a way to obtain the
    // vertex positions from the face handle timer_query.start();
    rx.for_each_vertex(DEVICE, [=] __device__(const VertexHandle& vh) mutable {
        const Eigen::Vector3<T> xi = x.template to_eigen<3>(vh);
    ...
    rx.for_each_vertex(DEVICE, [=] __device__(const VertexHandle& vh) mutable {
        ...
        for (int face = 0; face < 5; face++) {
            ...
            if (p_n < 0) {  // moving toward this wall
                alpha(vh) =
                    std::min(alpha(vh), T(0.9) * glm::dot(n, (xi - o)) / -p_n);
            }
        }
    });
