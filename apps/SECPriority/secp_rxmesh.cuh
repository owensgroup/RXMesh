    ...
    const std::shared_ptr<rxmesh::EdgeAttribute<T>>& edge_attr)
{
    ...
    auto edge_colors =
        ps_mesh->addEdgeScalarQuantity("Edges to Collapse", edgeColors);
    ...
    while (rx.get_num_vertices(true) > final_num_vertices) {
        ...
        rx.update_launch_box(
            ...
            [&](uint32_t v, uint32_t e, uint32_t f) {
                ...
                return pq.get_shmem_size(blockThreads) +
                       (e * sizeof(PriorityPair_t));
            });
            ...
            <<<launch_box.blocks,
               ...
               launch_box.smem_bytes_dyn>>>(rx.get_context(),
                                            *coords,
                                            ...
                                            pq.get_shmem_size(blockThreads));
        edge_priorities_timer.stop();
        e_priority_time += edge_priorities_timer.elapsed_millis();
        // cudaDeviceSynchronize();
        // RXMESH_TRACE("launch_box.smem_bytes_dyn = {}",
        // launch_box.smem_bytes_dyn); RXMESH_TRACE("pq.get_shmem_size = {}",
        // pq.get_shmem_size(blockThreads));
        ...
        pop_and_mark_edges_to_collapse<threads_per_block>
            <<<number_of_blocks, threads_per_block, shared_mem_bytes>>>(
                pq.get_mutable_device_view(), *e_pop_attr, pop_num_edges);
        ...
        while (!rx.is_queue_empty() &&
            ...
            secp<float, blockThreads><<<DIVIDE_UP(launch_box.blocks, 8),
                ...
                rx.get_context(), *coords, reduce_threshold, *e_pop_attr);
