                          ...
                          const FAttrT&         B2)
{
    ...
    if (!f.is_open()) {
        RXMESH_ERROR("read_raw_field() failed to open {}", file_name);
        return -1;
    }
    int num_f;
...
template <typename FAttrT>
void inline write_raw_field(const std::string&    file_name,
                            rxmesh::RXMeshStatic& rx,
                            FAttrT&               attr)
{
    using namespace rxmesh;
    ...
    int N     = attr.get_num_attributes();
    int num_f = rx.get_num_faces();
    ...
    for (int i = 0; i < num_f; i++) {
        for (int j = 0; j < 3 * N; j++) {
            ...
            FaceHandle fh = rx.map_to_local_face(i);
            f << attr(fh, j);
        }
    }
    f.close();
