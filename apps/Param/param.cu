...
#include "rxmesh/diff/diff_problem.h"
...
template <typename T>
void using_problem(RXMeshStatic& rx)
{
    constexpr int VariableDim = 2;

    using DiffProblemT = DiffProblem<T, VariableDim, VertexHandle>;

    DiffProblemT problem(rx);
    ...
    problem.template add_term<Op::FV, true>(
        rx, [=] __device__(const auto& fh, const VertexIterator& iter) {
            assert(iter[0].is_valid() && iter[1].is_valid() &&
                   iter[2].is_valid());

            using T = ACTIVE_TYPE(fh);
    ...
    RXMESH_INFO(
        "Paramterization: iterations ={}, time= {} (ms), "
