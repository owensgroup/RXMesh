...
GPUOrdering_V2::GPUOrdering_V2() : Gp(nullptr), Gi(nullptr), G_n(0), G_nnz(0)
{
}
    ...
    std::vector<int>& separator_nodes)
{
    ...
    spdlog::info(
        "Partition sizes: {} - {}", partition_1_ratio, partition_2_ratio);
#endif
    separator_nodes.clear();
    for (int i = 0; i < G_n; ++i) {
        if (graph_to_partition_map[i] == -1)
            continue;
        ...
        for (int nbr_ptr = Gp[i]; nbr_ptr < Gp[i + 1]; ++nbr_ptr) {
            ...
            if (i >= nbr_id)
                continue;
            if (graph_to_partition_map[nbr_id] == -1)
                continue;
    ...
    std::sort(separator_nodes.begin(), separator_nodes.end());
    separator_nodes.erase(std::unique(separator_nodes.begin(), separator_nodes.end()), separator_nodes.end());

#ifdef DEBUG
    std::vector<bool> visited(G_n, false);
    for (int i = 0 ; i < separator_nodes.size(); i++) {
        assert(visited[separator_nodes[i]] == false);
        visited[separator_nodes[i]] = true;
    }
    // Check whether all the separators are considered
    for (int i = 0; i < G_n; i++) {
        if (graph_to_partition_map[i] == -1)
            continue;
        for (int nbr_ptr = Gp[i]; nbr_ptr < Gp[i + 1]; ++nbr_ptr) {
            ...
            if (graph_to_partition_map[nbr_id] == -1)
                continue;
            ;
            if (graph_to_partition_map[i] != graph_to_partition_map[nbr_id]) {
                // One of the end points should be in the separator
                assert(std::find(separator_nodes.begin(),
                                 separator_nodes.end(),
                                 i) != separator_nodes.end() ||
                       std::find(separator_nodes.begin(),
                                 separator_nodes.end(),
                                 nbr_id) != separator_nodes.end());
            }
        }
    }
#endif
    ...
    std::vector<int>& separator_nodes)
{
    ...
    struct EdgeHash
    {
        ...
        {
            return std::hash<int>()(e.first) ^
                   (std::hash<int>()(e.second) << 1);
        }
    };
    ...
    assert(graph_to_partition_map.size() == G_n &&
           "graph_to_partition_map size should match G_n");
    for (int i = 0; i < graph_to_partition_map.size(); i++) {
        if (graph_to_partition_map[i] == -1)
            continue;
        ...
        for (int nbr_ptr = Gp[i]; nbr_ptr < Gp[i + 1]; ++nbr_ptr) {
            ...
            if (node_partition_id != nbr_partition_id) {
                ...
                Edge edge =
                    std::make_pair(std::min(i, nbr_id), std::max(i, nbr_id));
    ...
    assert(uncovered_edges.empty() &&
           "All cross-partition edges should be covered by the separator");
}


void GPUOrdering_V2::find_separator(std::vector<int>& graph_to_partition_map,
                                    std::vector<int>& separator_nodes)
{
    ...
    if (separator_finding_method == "basic") {
        ...
        spdlog::error("Invalid separator_finding_method: {}",
                      separator_finding_method);
    }
    // Check for correct separator
#ifdef DEBUG
    {
        // Check the correctness of separator_nodes
        spdlog::info("The separator size is {}", separator_nodes.size());
        std::vector<int> node_to_partition_with_separator =
            graph_to_partition_map;
        // Update the separator as third group
        for (int i = 0; i < separator_nodes.size(); i++) {
            node_to_partition_with_separator[separator_nodes[i]] = 2;
        }

        // Check whether there is an edge from the paritions to each other
        for (int i = 0; i < G_n; i++) {
            int partition_id_i = node_to_partition_with_separator[i];
            if (partition_id_i == -1)
                continue;

            for (int nbr_ptr = Gp[i]; nbr_ptr < Gp[i + 1]; ++nbr_ptr) {
                int nbr_id           = Gi[nbr_ptr];
                int partition_id_nbr = node_to_partition_with_separator[nbr_id];
                if (partition_id_nbr == -1)
                    continue;
                if (partition_id_i == partition_id_nbr) {
                    continue;
                } else {
                    // One of these partitions should be separator and the other
                    // should be not
                    if (partition_id_i == 2 || partition_id_nbr == 2) {
                        continue;
                    } else {
                        spdlog::error(
                            "The separator is wrong! partition {} should not "
                            "be connected to partition {}",
                            partition_id_i,
                            partition_id_nbr);
                        assert(false);
                        exit(1);
    ...
    if (separator_refinement_method == "patch_refinement") {
        ...
        separator_patch_refinement(graph_to_partition_map, separator_nodes);
        separator_redundancy_removal(graph_to_partition_map, separator_nodes);
    } else if (separator_refinement_method == "bipartite_graph_refinement") {
        separator_bipartite_graph_refinement(graph_to_partition_map,
                                             separator_nodes);
    } else if (separator_refinement_method == "nothing") {

    } else {
        spdlog::error("Invalid separator_refinement_method: {}",
                      separator_refinement_method);
    }
...
#ifdef DEBUG
    {
        ...
        std::vector<int> node_to_partition_with_separator =
            graph_to_partition_map;
        // Update the separator as the third group
        for (int i = 0; i < separator_nodes.size(); i++) {
            node_to_partition_with_separator[separator_nodes[i]] = 2;
        }

        // Check whether there is an edge from the paritions to each other
        spdlog::info("Checking separator correctness ... ");
        for (int i = 0; i < G_n; i++) {
            int partition_id_i = node_to_partition_with_separator[i];
            if (partition_id_i == -1)
                continue;
        ...
        spdlog::info("Separator is correct");
...
void GPUOrdering_V2::separator_patch_refinement(
    ...
    std::vector<int>& separator_nodes)
{
    // METIS-style refinement: Move separator nodes to partitions when
    // beneficial
    ...
    std::unordered_set<int> separator_set(separator_nodes.begin(),
                                          separator_nodes.end());
    ...
    while (improvement) {
        ...
        for (int node : separator_nodes) {
            ...
            for (int target_partition = 0; target_partition < 2;
                 target_partition++) {
        ...
        separator_nodes.clear();
        separator_nodes.insert(
            separator_nodes.end(), separator_set.begin(), separator_set.end());
    }

    if (initial_separator_size > separator_nodes.size()) {
        spdlog::info(
            "Patch refinement reduced separator from {} to {} in {} iterations",
    ...
    std::vector<int>& separator_nodes)
{
    ...
    while (nodes_removed) {
        ...
        std::vector<std::pair<int, int>>
            node_degrees;  // pair of (degree, node_id)
        for (int node : separator_nodes) {
        ...
        if (nodes_removed) {
            separator_nodes.clear();
            separator_nodes.insert(separator_nodes.end(),
                                   separator_set.begin(),
                                   separator_set.end());
        }
    }
...
void GPUOrdering_V2::separator_bipartite_graph_refinement(
    std::vector<int>& graph_to_partition_map,
    std::vector<int>& separator_nodes)
{
    ...
    // Overall flow:
    // Step 1: Compute the node_to_partition for the separator nodes
    // Step 2: Extract the bipartite graph from the separator nodes
    // Step 3: Use the min_vertex_cover_bipartite to compute the max matching
    ...
    //============
    std::sort(separator_nodes.begin(), separator_nodes.end());
    std::vector<int> local_graph_to_partition(
        separator_nodes.size(), -1);  // This is a local node to partition map
    for (int i = 0; i < separator_nodes.size(); i++) {
        local_graph_to_partition[i] = graph_to_partition_map[separator_nodes[i]];
        assert(local_graph_to_partition[i] >= 0);
    }
#ifdef DEBUG
    // Make sure that there is only two partitions
    std::unordered_set<int> partition_unique_ids;
    for (int i = 0; i < local_graph_to_partition.size(); i++) {
        partition_unique_ids.insert(local_graph_to_partition[i]);
    }
    assert(partition_unique_ids.size() == 2 &&
           "There should be only two partitions");
#endif
    //=========== Step 2: Extract the bipartite graph from the separator nodes
    //============
    ...
    for (int i = 0; i < separator_nodes.size(); i++) {
        separator_node_to_local_node[separator_nodes[i]] = i;
    }
    // Extract the bipartite graph
    for (int i = 0; i < separator_nodes.size(); i++) {
        for (int nbr_ptr = Gp[separator_nodes[i]];
             nbr_ptr < Gp[separator_nodes[i] + 1];
             ++nbr_ptr) {
            int nbr_id = Gi[nbr_ptr];
            // If nbr is a separator node consider the edge
            int local_nbr_id = -1;
            if (separator_node_to_local_node.find(nbr_id) !=
                separator_node_to_local_node.end()) {
                local_nbr_id = separator_node_to_local_node[nbr_id];
                //If two nodes are from the same partition
                if (local_graph_to_partition[i] == local_graph_to_partition[local_nbr_id]) {
                    continue;
                }
            } else {
                //The local node is not part of the separator set
                local_nbr_id = -1;
                continue;
            }

            if (i < local_nbr_id) {
                triplets.push_back(Eigen::Triplet<int>(i, local_nbr_id, 1));
                triplets.push_back(Eigen::Triplet<int>(local_nbr_id, i, 1));
            }
        }
    }
    bipartite_graph.resize(separator_nodes.size(), separator_nodes.size());
    bipartite_graph.setFromTriplets(triplets.begin(), triplets.end());
    bipartite_graph.makeCompressed();
#ifdef DEBUG
    // Check whether the graph is bipartite
    for (int i = 0; i < bipartite_graph.rows(); i++) {
        for (int nbr_ptr = bipartite_graph.outerIndexPtr()[i];
             nbr_ptr < bipartite_graph.outerIndexPtr()[i + 1];
             ++nbr_ptr) {
            int nbr_id = bipartite_graph.innerIndexPtr()[nbr_ptr];
            if (local_graph_to_partition[i] == local_graph_to_partition[nbr_id]) {
    ...
    int initial_separator_size = separator_nodes.size();
    std::vector<int> tmp = separator_nodes;
    separator_nodes.clear();
    //Converting the local min_vertex_cover to global one
    for (int i = 0; i < min_vertex_cover.size(); i++) {
        separator_nodes.push_back(tmp[min_vertex_cover[i]]);
    }
    int final_separator_size = separator_nodes.size();
#ifdef DEBUG
                                                  ...
                                                  int decomposition_node_id)
{
    assert(decomposition_node_id <
           max_match_tree.get_number_of_decomposition_nodes());
    auto& decomposition_node =
        max_match_tree.decomposition_nodes[decomposition_node_id];
    ...
    // #pragma omp parallel
    {
        for (int l = 0; l < wavefront_levels; l++) {
            ...
            for (int node_idx = start_level_idx; node_idx < end_level_idx;
                ...
                std::vector<int>& assigned_patches =
                    decomposition_info_stack[node_idx]
                        .decomposition_node_patches;
                ...
                if (l == wavefront_levels - 1) {
                    ...
                    //     //print a warning that in the leaf node, it is better
                    //     to have only one patch spdlog::warn("In the leaf
                    //     decomposition node, it is better to not have more
                    //     than one patch"); spdlog::warn("For node {} in level
                    //     {}: #Assigned patches: {}", node_idx, l,
                    //     assigned_patches.size());
                    // }
                ...
                //=========== Step 1: Divide the patches into two parts (left
                //and right) ============ Compute the local quotient graph from
                // the assigned patches
                ...
                // Compute the two parts of the local quotient graph for the
                // next decomposition node
                ...
                for (int i = 0; i < local_node_to_partition.size(); i++) {
                    ...
                    assert(global_patch_id >= 0 &&
                           global_patch_id < patch_nodes.size() &&
                           "Invalid patch ID");
                    // Check if the patch has non-separator nodes - IMPORTANT
                    bool is_valid_patch = false;
                    for (int j = 0;
                         j < patch_nodes[global_patch_id].nodes.size();
                         ++j) {
                        if (!max_match_tree.is_separator
                                 [patch_nodes[global_patch_id].nodes[j]]) {
                            is_valid_patch = true;
                            break;
                        }
                    }
                ...
                assert((left_assigned.size() + right_assigned.size() +
                        skipped_invalid_patches) == assigned_patches.size() &&
                       "All patches should be accounted for");

                //=========== Step 2: Compute the separator between these two
                //parts ============
                // Compute the separator between the two parts of the local
                // quotient graph
                ...
                for (int i = 0; i < assigned_patches.size(); i++) {
                    ...
                    for (int j = 0; j < patch.nodes.size(); j++) {
                        if (max_match_tree.is_separator[patch.nodes[j]])
                            continue;
                ...
                for (int i = 0; i < separator.size(); ++i) {
                    ...
                    assert(sep_node >= 0 && sep_node < G_n &&
                           "Separator node index out of bounds");
                    if (max_match_tree.is_separator[sep_node] != false){
                        spdlog::info("Node {} already marked as separator.", sep_node);
                        assert(false);
                    }
                ...
                // decompositon node ============ Compute the permutation for
                // the
                // separator
                ...
                if (!separator.empty()) {
                    ...
                    // spdlog::info("For decomposition node {} in level {},
                    // #Assigned patches: {}, #Separator nodes / total nodes:
                    // {}",
                    //              node_idx, l, assigned_patches.size(),
                    //              separator.size() * 1.0 / G_n);
                ...
                if (left_node_idx != -1) {
                    ...
                    decomposition_info_stack[left_node_idx]
                        .decomposition_node_patches = left_assigned;
                }
                ...
                if (right_node_idx != -1) {
                    ...
                    decomposition_info_stack[right_node_idx]
                        .decomposition_node_parent_id = decomposition_node_id;
                    decomposition_info_stack[right_node_idx]
                        .decomposition_node_patches = right_assigned;
                }
            }
        }
    }
...
void GPUOrdering_V2::step2_create_hierarchical_partitioning_and_permute()
{
    ...
    spdlog::info("Using separator finding method: {}",
                 separator_finding_method);
    spdlog::info("Using separator refinement method: {}",
                 separator_refinement_method);
...
// #ifdef DEBUG
    ...
    for (int i = 0; i < max_match_tree.decomposition_nodes.size(); ++i) {
        ...
        for (int j = 0; j < max_match_tree.decomposition_nodes[i].dofs.size();
             ++j) {
    ...
    int total_number_of_nodes_exist =
        std::count(node_exist.begin(), node_exist.end(), true);
    ...
    for (int i = 0; i < max_match_tree.is_separator.size(); ++i) {
        if (max_match_tree.is_separator[i])
            total_flagged_separator_nodes++;
    }
...
void GPUOrdering_V2::assemble_permutation(int decomposition_node_id,
                                          std::vector<int>& perm)
{
    assert(decomposition_node_id >= 0 &&
           decomposition_node_id < max_match_tree.decomposition_nodes.size() &&
           "Invalid decomposition_node_id");
    auto& node = max_match_tree.decomposition_nodes[decomposition_node_id];
    assert(node.dofs.size() == node.local_new_labels.size() &&
           "DOFs and labels size mismatch");
    ...
    if (node.left_node_idx != -1) {
        assert(node.left_node_idx < max_match_tree.decomposition_nodes.size() &&
               "Invalid left child index");
        assemble_permutation(node.left_node_idx, perm);
    }
    if (node.right_node_idx != -1) {
        assert(node.right_node_idx <
                   max_match_tree.decomposition_nodes.size() &&
               "Invalid right child index");
