...
#define DOUBLE
#define REAL
#include "parth_super_numeric_worker.h"
#define COMPLEX
#include "parth_super_numeric_worker.h"
#define ZOMPLEX
#include "parth_super_numeric_worker.h"
...
)
{
    ...
    SuperMap = (int32_t*)Common->Iwork ;  // size n
    Map = (int32_t*) Common->Flag ;        // size n, use Flag as workspace for Map array
    for (i = 0 ; i < n ; i++)
    {
        Map [i] = EMPTY ;
    }
    ...
    switch ((A->xtype + A->dtype) % 8)
    {
        case CHOLMOD_REAL    + CHOLMOD_SINGLE:
            ok = rs_parth_super_numeric_worker (A, F, s_beta, L, C, Common, *this) ;
            break ;

        case CHOLMOD_COMPLEX + CHOLMOD_SINGLE:
            ok = cs_parth_super_numeric_worker (A, F, s_beta, L, C, Common, *this) ;
            break ;

        case CHOLMOD_ZOMPLEX + CHOLMOD_SINGLE:
            // A is zomplex, but L is complex
            ok = zs_parth_super_numeric_worker (A, F, s_beta, L, C, Common, *this) ;
            break ;

        case CHOLMOD_REAL    + CHOLMOD_DOUBLE:
            ok = rd_parth_super_numeric_worker (A, F, beta, L, C, Common, *this) ;
            break ;

        case CHOLMOD_COMPLEX + CHOLMOD_DOUBLE:
            ok = cd_parth_super_numeric_worker (A, F, beta, L, C, Common, *this) ;
            break ;

        case CHOLMOD_ZOMPLEX + CHOLMOD_DOUBLE:
            // A is zomplex, but L is complex
            ok = zd_parth_super_numeric_worker (A, F, beta, L, C, Common, *this) ;
            break ;
    }
