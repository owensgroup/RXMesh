...
static int TEMPLATE(parth_super_numeric_worker)(
    // input:
    ...
    PARTH_SOLVER::ParthSolverAPI& solver)
{
    ...
    Int * Super, *Head, *Ls, *Lpi, *Lpx, *Map, *SuperMap, *RelativeMap, *Next,
        ...
        *Previous;
    Int nsuper, n, s, k1, k2, nscol, psi, psx, psend, nsrow, pj, d, kd1, kd2,
        info, ndcol, ndrow, pdi, pdx, pdend, pdi1, pdi2, ndrow1, ndrow2,
        dancestor, sparent, dnext, nsrow2, ndrow3, stype, Apacked, Fpacked,
        repeat_supernode, nscol2, ss, tail, nscol_new = 0;
    info = 0;
...
#if defined(CHOLMOD_HAS_CUDA) && defined(CHOLMOD_INT64) && defined(DOUBLE)
    ...
    C = (Real*)Cwork->x;  // workspace of size L->maxcsize
    ...
    Map =
        (int32_t*)Common->Flag;  // size n, use Flag as workspace for Map array
    Head = (int32_t*)Common->Head;  // size n+1, only Head [0..nsuper-1] used

    Ls  = (int32_t*)L->s;
    Lpi = (int32_t*)L->pi;
    Lpx = (int32_t*)L->px;

    Super = (int32_t*)L->super;
...
// fprintf (stderr, "local useGPU %d\n", useGPU) ;
    ...
    Int ii;
#pragma omp parallel for num_threads(nthreads) if (n > 128) schedule(static)
    for (ii = 0; ii < n; ii++) {
        Map[ii] = EMPTY;
    }
    ...
    std::vector<std::vector<int>>    map_list(num_cores,
                                           std::vector<int>(n, EMPTY));
    ...
    std::vector<std::vector<int>>    RelativeMap_list(num_cores,
                                                   std::vector<int>(n));
    std::vector<int>                 Neighbors(nsuper);
...
#pragma omp parallel for schedule(dynamic)                                  \
    num_threads(num_cores) private(j,                                       \
                                       i,                                   \
                                       k,                                   \
                                       s,                                   \
                                       p,                                   \
    ...
    firstprivate(repeat_supernode, nscol_new) default(none)
            for (int part_ptr = solver.level_ptr[l];
                ...
                for (int s_ptr = solver.part_ptr[part_ptr];
                     s_ptr < solver.part_ptr[part_ptr + 1];
                     s_ptr++) {
    ...
    for (s = 0; s < nsuper; s++) {
                    ...
                    psend =
                        Lpi[s + 1];  // pointer just past last row of s in Ls
...
#pragma omp parallel for num_threads(nthreads) \
    schedule(static) if (pend - psx > 1024)
                    ...
                    // If row i is the kth row in s, then Map [i] = k.
                    // Similarly, if column j is the kth column in s, then  Map
                    // [j] = k.
                    ...
                    Int k;
#ifndef WAVEFRONT
#pragma omp parallel for num_threads(nthreads) if (nsrow > 128)
#endif
                    for (k = 0; k < nsrow; k++) {
                        PRINT1(("  " ID " map " ID "\n", Ls[psi + k], k));
                        Map[Ls[psi + k]] = k;
                    ...
                    if (stype != 0) {
                        Int pfirst = Ap[k1];
                        Int plast =
                            (Apacked) ? (Ap[k2]) : (pfirst + Anz[k2 - 1]);
                        work = (double)(plast - pfirst);
                    } else {
                        Int pfirst = Fp[k1];
                        Int plast =
                            (Fpacked) ? (Fp[k2]) : (pfirst + Fnz[k2 - 1]);
                        work = (double)(plast - pfirst);
                    }
                    nthreads = cholmod_nthreads(work, Common);
#endif
#ifndef WAVEFRONT
#pragma omp parallel for num_threads(nthreads) if (k2 - k1 > 64)
#endif
                    for (k = k1; k < k2; k++) {
                        if (stype != 0) {
                            ...
                            for (; p < pend; p++) {
                                ...
                                Int i = Ai[p];
                                if (i >= k) {
                                    // If the test is false, the numeric
                                    // factorization of A is undefined.  The
                                    // test does not detect all invalid entries,
                                    // only some of them (when debugging is
                                    // enabled, and Map is cleared after each
                                    // step, then all entries not in the pattern
                                    // of L are detected).
                                    Int imap = Map[i];
                    ...
                    if (beta[0] != 0.0) {
                        ...
                        for (Int k = k1; k < k2; k++) {
                            ...
                            pk += nsrow +
                                  1;  // advance to the next diagonal entry
                        }
                    }

                    PRINT1(("Supernode with just A: repeat: " ID "\n",
                            repeat_supernode));
                    DEBUG(CHOLMOD(dump_super)(
                        s, Super, Lpi, Ls, Lpx, Lx, L->dtype, L_ENTRY, Common));
                    PRINT1(("\n\n"));
                    ...
                    if (!repeat_supernode) {
                        ...
                        // Save the list of pending descendants in case s is not
                        // positive definite.  Also save Lpos for each
                        // descendant d, so that we can find which part of d is
                        // used to update s.
            ...
            (dnext != EMPTY)
#endif
                    {
                        ...
                        if (useGPU) {
                            ...
                            // Conditionally select the next descendant
                            // supernode to
                            //  assemble.
                            //   + first, select the largest descendant
                            //   + subsequently, if gpu host buffers are
                            //   available, select
                            //     the largest remaining descendant for assembly
                            //     on the GPU
                            //   + otherwise select the smallest remaining
                            //   descendant for
                            //     assembly on the CPU
                            //
                            // The objective is to keep the GPU busy assembling
                            // the largest descendants, and simultaneously keep
                            // the CPU busy assembling the smallest descendants.
                            //
                            // As this is called for every descendent supernode,
                            // moving this code to t_cholmod_gpu incurs
                            // substantial overhead - ~20 GF/s on audikw_1 - so
                            // it is being left here.

                            iHostBuff = (Common->ibuffer) %
                            ...
                            if (idescendant > 0) {
                                if (GPUavailable == -1 || skips > 0) {
                                    ...
                                    if (cuErr == cudaSuccess) {
                                        // buffers are available, so assemble a
                                        // large descendant (anticipating that
                                        // this will be assembled on the GPU)
                                        d            = dlarge;
                                        dlarge       = Next[dlarge];
                                        GPUavailable = 1;
                                        skips        = 0;
                                    } else {
                                        // buffers are not available, so the GPU
                                        // is busy, so assemble a small
                                        // descendant (anticipating that it will
                                        // be assembled on the host)
                                        d            = dsmall;
                                        dsmall       = Previous[dsmall];
                                        GPUavailable = 0;

                                        // if the GPUs are busy, then do this
                                        // many supernodes on the CPU before
                                        // querying GPUs again.
                        ...
                        pdend =
                            Lpi[d +
                                1];  // pointer just past last row of d in Ls
                        ...
                        //------------------------------------------------------------------
                        // find the range of rows of d that affect rows k1 to
                        // k2-1 of s
                        //------------------------------------------------------------------

                        Int p = Lpos[d];  // offset of 1st row of d affecting s
                        Int pdi1 =
                            pdi + p;  // ptr to 1st row of d affecting s in Ls
                        Int pdx1 =
                            pdx + p;  // ptr to 1st row of d affecting s in Lx

                        // there must be at least one row remaining in d to
                        // update s
                        ASSERT(pdi1 < pdend);
                        ...
                        // rows Ls [pdi1 ... pdi2-1] are in the range k1 to
                        // k2-1.  Since d affects s, this set cannot be empty.
                        ...
                        // C = L (k1:n-1, kd1:kd2-1) * L (k1:k2-1, kd1:kd2-1)',
                        // except that k1:n-1 refers to all of the rows in L,
                        // but many of the rows are all zero.  Supernode d holds
                        // columns kd1 to kd2-1 of L. Nonzero rows in the range
                        // k1:k2-1 are in the list Ls [pdi1 ... pdi2-1], of size
                        // ndrow1.  Nonzero rows in the range k2:n-1 are in the
                        // list Ls [pdi2 ... pdend], of size ndrow2.  Let L1 = L
                        // (Ls [pdi1 ... pdi2-1], kd1:kd2-1), and let L2 = L (Ls
                        // [pdi2 ... pdend],  kd1:kd2-1).  C is
                        // ndrow2-by-ndrow1. Let C1 be the first ndrow1 rows of
                        // C and let C2 be the last ndrow2-ndrow1 rows of C.
                        // Only the lower triangular part of C1 needs to be
                        // computed since C1 is symmetric.

                        // maxcsize is the largest size of C for all pairs (d,s)
                        ASSERT(ndrow2 * ndrow1 <= ((Int)L->maxcsize));

                        // compute leading ndrow1-by-ndrow1 lower triangular
                        // block of C, C1 = L1*L1'
                        ...
                        if (useGPU) {
                            ...
                            if (GPUavailable == 1) {
                                if (ndrow2 * L_ENTRY >= CHOLMOD_ND_ROW_LIMIT &&
                                    ...
                                    if (!mapCreatedOnGpu) {
                                        TEMPLATE2(
                                            CHOLMOD(gpu_initialize_supernode))
                                        (Common, nscol, nsrow, psi, gpu_p);
                                        mapCreatedOnGpu = 1;
                                    }
                                } else {
                                    // we've reached the limit of GPU-eligible
                                    // descendants flag to stop stop performing
                                    // cudaEventQueries
                                    GPUavailable = -1;
                                }
                            }
                        ...
                        if (!useGPU || GPUavailable != 1 ||
                            !TEMPLATE2(CHOLMOD(gpu_updateC))(ndrow1,
                                                             ndrow2,
                                                             ndrow,
                                                             ndcol,
                                                             nsrow,
            ...
            SUITESPARSE_BLAS_cherk("L",
                            ...
                            // compute remaining (ndrow2-ndrow1)-by-ndrow1 block
                            // of C, C2 = L2*L1'
                            if (ndrow3 > 0) {
...
#ifdef _OPENMP
                            int nthreads =
                                cholmod_nthreads((double)ndrow2, Common);
#endif

                            Int i;
#ifndef WAVEFRONT
#pragma omp parallel for num_threads(nthreads) if (ndrow2 > 64)
#endif
                            for (i = 0; i < ndrow2; i++) {
                                RelativeMap[i] = Map[Ls[pdi1 + i]];
                                ASSERT(RelativeMap[i] >= 0 &&
                                       RelativeMap[i] < nsrow);
                            }

                            //--------------------------------------------------------------
                            // assemble C into supernode s using the relative
                            // map
                            //--------------------------------------------------------------

#ifdef _OPENMP
                            double work =
                                (double)ndcol * (double)ndrow2 * L_ENTRY;
                            nthreads = cholmod_nthreads(work, Common);
#endif

                            Int j;
#ifndef WAVEFRONT
#pragma omp parallel for num_threads(nthreads) if (ndrow1 > 64)
#endif
                            for (j = 0; j < ndrow1; j++)  // cols k1:k2-1
                            {
                                ASSERT(RelativeMap[j] == Map[Ls[pdi1 + j]]);
                                ASSERT(RelativeMap[j] >= 0 &&
                                       RelativeMap[j] < nscol);
                                Int px = psx + RelativeMap[j] * nsrow;
                                for (Int i = j; i < ndrow2; i++)  // rows k1:n-1
                                {
                                    ASSERT(RelativeMap[i] == Map[Ls[pdi1 + i]]);
                                    ASSERT(RelativeMap[i] >= j &&
                                           RelativeMap[i] < nsrow);
                            ...
                            supernodeUsedGPU =
                                1;  // GPU was used for this supernode
                            Common->ibuffer++;  // gpu_updateC is asynchronous,
                                                // so use the next host buffer
                                                // for the next supernode
                            Common->ibuffer = Common->ibuffer %
                                              (CHOLMOD_HOST_SUPERNODE_BUFFERS *
                                               CHOLMOD_DEVICE_STREAMS);
                        }
#endif
...
#pragma omp critical
...
#endif
                            if (!repeat_supernode) {
                                // If node s is being repeated, Head [dancestor]
                                // has already been cleared (set to EMPTY).  It
                                // must remain EMPTY.  The dancestor will not be
                                // factorized since the factorization terminates
                                // at node s.
                                Lpos[d] = pdi2 - pdi;
                                if (Lpos[d] < ndrow) {
                                    dancestor = SuperMap[Ls[pdi2]];
                                    ASSERT(dancestor > s && dancestor < nsuper);
                                    // place d in the link list of its next
                                    // ancestor
                    ...
                    // The current supernode s is ready to factorize.  It has
                    // been updated by all descendant supernodes.  Let S = the
                    // current supernode, which holds rows k1:n-1 and columns
                    // k1:k2-1 of the updated matrix.   It splits into two
                    // parts:  the square diagonal block S1, and the rectangular
                    // part S2.  Here, S1 is factorized into L1*L1' and
                    // overwritten by L1.
                    //
                    // If supernode s is being repeated, only factorize it up to
                    // but not including the column containing the problematic
                    // entry.

                    nscol2 = (repeat_supernode) ? (nscol_new) : (nscol);

#if defined(CHOLMOD_HAS_CUDA) && defined(CHOLMOD_INT64) && defined(DOUBLE)
                    if (!useGPU || !supernodeUsedGPU ||
                        !TEMPLATE2(CHOLMOD(gpu_lower_potrf))(
                            nscol2, nsrow, psx, Lx, &info, Common, gpu_p))
#endif
                    {
...
#pragma omp atomic update
                    ...
                    if (repeat_supernode) {
                        // the leading part has been refactorized; it must have
                        // succeeded
                        info = 0;
                    ...
                    // info is set to one in SUITESPARSE_LAPACK_*potrf if
                    // blas_ok is FALSE. It is set to zero in dpotrf/zpotrf if
                    // the factorization was successful.
                    CHECK_FOR_BLAS_INTEGER_OVERFLOW;

                    if (info != 0) {
                        // Matrix is not positive definite.  dpotrf/zpotrf do
                        // NOT report an error if the diagonal of L has NaN's,
                        // only if it has a zero.
                        if (Common->status == CHOLMOD_OK) {
                            ERROR(CHOLMOD_NOT_POSDEF,
                                  "matrix not positive definite");
                        }

                        // L->minor is the column of L that contains a zero or
                        ...
                        // If L is indefinite, it still contains useful
                        // information. Supernodes 0 to s-1 are valid, similar
                        // to MATLAB [R,p]=chol(A), where the 1-based p is
                        // identical to the 0-based L->minor.  Since L->minor is
                        // in the current supernode s, it and any columns to the
                        // left of it in supernode s are also all zero.  This
                        // differs from [R,p]=chol(A), which contains nonzero
                        // rows 1 to p-1.  Fix this by setting repeat_supernode
                        // to TRUE, and repeating supernode s.
                        //
                        // If Common->quick_return_if_not_posdef is true, then
                        // the entire supernode s is not factorized; it is left
                        // as all zero.

                        if (info == 1 || Common->quick_return_if_not_posdef) {
                            // If the first column of supernode s contains a
                            // zero or negative diagonal entry, then it is
                            // already properly set to zero.  Also, info will be
                            // 1 if integer overflow occured in the BLAS.
                            Head[s] = EMPTY;
#if defined(CHOLMOD_HAS_CUDA) && defined(CHOLMOD_INT64) && defined(DOUBLE)
                            if (useGPU) {
                            ...
                            // Repeat supernode s, but only factorize it up to
                            // but not including the column containing the
                            // problematic diagonal entry.
                    ...
                    //----------------------------------------------------------------------
                    // compute the subdiagonal block and prepare supernode for
                    // its parent
                    //----------------------------------------------------------------------

                    nsrow2 = nsrow - nscol2;
                    if (nsrow2 > 0) {
                        // The current supernode is columns k1 to k2-1 of L. Let
                        // L1 be the diagonal block (factorized by dpotrf/zpotrf
                        // above; rows/cols k1:k2-1), and L2 be rows k2:n-1 and
                        // columns k1:k2-1 of L.  The triangular system to solve
                        // is L2*L1' = S2, where S2 is overwritten with L2. More
                        // precisely, L2 = S2 / L1' in MATLAB notation.
...
#endif
                        {
                            ...
                            SUITESPARSE_BLAS_dtrsm(
                                ...
                                one,     // ALPHA: 1
                                Lx + L_ENTRY * psx,
                                nsrow,  // A, LDA: L1, nsrow
                                Lx + L_ENTRY *
                                         (psx + nscol2),  // B, LDB, L2, nsrow
                                nsrow,
                                Common->blas_ok);
            ...
            SUITESPARSE_BLAS_ctrsm(
                ...
                Common->blas_ok);
                        ...
                        if (!repeat_supernode) {
                            // Lpos [s] is offset of first row of s
                    ...
                    Head[s] =
                        EMPTY;  // link list for supernode s no longer needed

                    // clear the Map (debugging only, to detect changes in
                    // pattern of A)
                    ...
                    if (repeat_supernode) {
                        // matrix is not positive definite; finished clean-up
                        // for supernode containing negative diagonal
