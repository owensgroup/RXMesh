...
int ParthSolverAPI::cholmod_factorize_custom
(
    // input:
...
int ParthSolverAPI::cholmod_factorize_p_custom
(
    // input:
    cholmod_sparse *A,  // matrix to factorize
    ...
    if (L->is_super)
    {
        ...
        else
        {
            ...
            if (stype > 0)
            {
                ...
                A1 = CHOLMOD(ptranspose) (A, 2, (int32_t*)L->Perm, NULL, 0, Common) ;
                S = A1 ;
            }
            else if (stype < 0)
            {
                // A2 = triu (PAP')
                // workspace: Iwork (2*nrow)
                A2 = CHOLMOD(ptranspose) (A, 2, (int32_t*)L->Perm, NULL, 0, Common) ;
            ...
            else
            {
                // F = A(p,f)'
                // workspace: Iwork (nrow if no fset; MAX (nrow,ncol) if fset)
                A1 = CHOLMOD(ptranspose) (A, 2, (int32_t*)L->Perm, fset, fsize, Common) ;
                F = A1 ;
                // S = F'
                // workspace: Iwork (nrow)
