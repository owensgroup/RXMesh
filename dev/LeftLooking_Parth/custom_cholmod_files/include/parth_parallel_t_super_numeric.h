...
static int TEMPLATE(parth_super_numeric_parallel)(
    /* ---- input ---- */
...
#pragma omp parallel for num_threads(cholmod_num_threads) if (n > 128)     \
  ...
  std::cout << "APPLYING THIS WAVEFRONT REGION PARALLELISM" << std::endl;
  /* ---------------------------------------------------------------------- */
  ...
  for (int l = 0; l < solver.level_ptr.size() - 1; l++) {
    ...
    if (num_parts > 1) {
#pragma omp parallel for schedule(dynamic) num_threads(num_cores) private(     \
        j, i, k, s, p, pend, k1, k2, nscol, psi, psx, psend, nsrow, d, kd1,    \
            kd2, info, ndcol, ndrow, pdi, pdx, pdend, pdi1, pdi2, pdx1,        \
            ndrow1, ndrow2, px, dancestor, sparent, dnext, nsrow2, ndrow3, pk, \
            pf, pfend, q, imap, nscol2, ss, tail, pj, tstart)                  \
    shared(nsuper, n, solver, Super, Lpi, Lpx, Lx, Ap, Ax, Ai, Fp, Fx, Fi,     \
               Fnz, Fz, Anz, beta, one, zero, Common, Head, Next, Lpos,        \
               SuperMap, L, Ls, map_list, C_list, RelativeMap_list, Lpos_save, \
               Next_save, std::cout, std::cerr, Az, stype, Fpacked, Apacked,   \
               begin_marked, end_marked, l)                                    \
    ...
    dnext = Head[s];
      ...
      d = dnext;
            ...
            if (!useGPU || GPUavailable != 1 ||
                !TEMPLATE2(CHOLMOD(gpu_updateC))(ndrow1, ndrow2, ndrow, ndcol,
                                                 nsrow, pdx1, pdi1, Lx, C,
                                                 Common, gpu_p))
#endif
            {
...
#pragma omp parallel for private(p, pend, pfend, pf, i, j, imap, q)            \
    num_threads(cholmod_num_threads) if (k2 - k1 > 64)
          ...
          // #endif
          //          num_cores = 1;
          std::sort(Neighbors.data(), Neighbors.data() + nbr_cnt);
#pragma omp parallel for schedule(dynamic)                                     \
    num_threads(num_cores) private(d, kd1, kd2, ndcol, pdi, pdx, pdend, ndrow, \
                                       p, pdi1, pdx1, pdi2, ndrow1, ndrow2,    \
                                       ndrow3, i, px, dancestor, j, q)         \
    shared(Neighbors, RelativeMap_list, C_list, Super, Lpi, Lpx, Lpos, one,    \
               Lx, zero, nsrow, Map, repeat_supernode, psx, Common, Next,      \
               Head, nbr_cnt, SuperMap, Ls, k2) default(none)
          for (int d_ptr = 0; d_ptr < nbr_cnt; d_ptr++) {

#ifdef GPU_BLAS

            if (useGPU) {

              /* Conditionally select the next
               * descendant supernode to assemble.
               *   + first, select the largest
               * descendant
               *   + subsequently, if gpu host buffers
               * are available, select the largest
               * remaining descendant for assembly on
               * the GPU
               *   + otherwise select the smallest
               * remaining descendant for assembly on
               * the CPU
               *
               * The objective is to keep the GPU busy
               * assembling the largest descendants,
               * and simultaneously keep the CPU busy
               * assembling the smallest descendants.
               *
               * As this is called for every
               * descendent supernode, moving this
               * code to t_cholmod_gpu incurs
               * substantial overhead - ~20 GF/s on
               * audikw_1 - so it is being left here.
               */
              ...
              if (idescendant > 0) {
                if (GPUavailable == -1 || skips > 0) {
                  ...
                  cuErr = cudaEventQuery(Common->updateCBuffersFree[iHostBuff]);
                  if (cuErr == cudaSuccess) {
                    /* buffers are available, so
                     * assemble a large descendant
                     * (anticipating that this will be
                     * assembled on the GPU) */
                    d = dlarge;
                    dlarge = Next[dlarge];
                    GPUavailable = 1;
                    ...
                    /* buffers are not available, so
                     * the GPU is busy, so assemble a
                     * small descendant (anticipating
                     * that it will be assembled on
                     * the host) */
                    d = dsmall;
                    dsmall = Previous[dsmall];
                    GPUavailable = 0;

                    /* if the GPUs are busy, then do
                     * this many supernodes on the CPU
                     * before querying GPUs again. */
                    skips = CHOLMOD_GPU_SKIP;
                  }
                }
              }
...
#else
            /* GPU module not installed at compile
             * time */
            d = Neighbors[d_ptr];
#endif
            /* --------------------------------------------------------------
            ...
            /* --------------------------------------------------------------
             */
            auto *RelativeMap = RelativeMap_list[omp_get_thread_num()].data();
            auto *C = C_list[omp_get_thread_num()].data();

            kd1 = Super[d]; /* d contains cols kd1 to
                               kd2-1 of L */
            kd2 = Super[d + 1];
            ndcol = kd2 - kd1;   /* # of columns in all of d */
            pdi = Lpi[d];        /* pointer to first row of d
                                    in Ls */
            pdx = Lpx[d];        /* pointer to first row of d
                                    in Lx */
            pdend = Lpi[d + 1];  /* pointer just past last
                                    row of d in Ls */
            ndrow = pdend - pdi; /* # rows in all of d */
            ...
            /* --------------------------------------------------------------
             */
            /* find the range of rows of d that affect
             * rows k1 to k2-1 of s */
            /* --------------------------------------------------------------
             */

            p = Lpos[d];    /* offset of 1st row of d
                               affecting s */
            pdi1 = pdi + p; /* ptr to 1st row of d
                               affecting s in Ls */
            pdx1 = pdx + p; /* ptr to 1st row of d
                               affecting s in Lx */

            /* there must be at least one row
             * remaining in d to update s */
            ASSERT(pdi1 < pdend);
            PRINT1(("Lpos[d] " ID " pdi1 " ID " Ls[pdi1] " ID "\n", Lpos[d],
                    pdi1, Ls[pdi1]));
            ASSERT(Ls[pdi1] >= k1 && Ls[pdi1] < k2);
            ...
            /* rows Ls [pdi1 ... pdi2-1] are in the
             * range k1 to k2-1.  Since d affects s,
             * this set cannot be empty. */
            ASSERT(pdi1 < pdi2 && pdi2 <= pdend);
            PRINT1(("ndrow1 " ID " ndrow2 " ID "\n", ndrow1, ndrow2));
            DEBUG(for (p = pdi1; p < pdi2; p++)
                      PRINT1(("Ls[" ID "] " ID "\n", p, Ls[p])));
            ...
            /* --------------------------------------------------------------
             */
            /* construct the update matrix C for this
             * supernode d */
            /* --------------------------------------------------------------
             */

            /* C = L (k1:n-1, kd1:kd2-1) * L (k1:k2-1,
             * kd1:kd2-1)', except that k1:n-1 refers
             * to all of the rows in L, but many of
             * the rows are all zero.  Supernode d
             * holds columns kd1 to kd2-1 of L.
             * Nonzero rows in the range k1:k2-1 are
             * in the list Ls [pdi1 ... pdi2-1], of
             * size ndrow1.  Nonzero rows in the range
             * k2:n-1 are in the list Ls [pdi2 ...
             * pdend], of size ndrow2.  Let L1 = L (Ls
             * [pdi1 ... pdi2-1], kd1:kd2-1), and let
             * L2 = L (Ls [pdi2 ... pdend],
             * kd1:kd2-1).  C is ndrow2-by-ndrow1. Let
             * C1 be the first ndrow1 rows of C and
             * let C2 be the last ndrow2-ndrow1 rows
             * of C.  Only the lower triangular part
             * of C1 needs to be computed since C1 is
             * symmetric.
             */

            /* maxcsize is the largest size of C for
             * all pairs (d,s) */
            ASSERT(ndrow2 * ndrow1 <= ((Int)L->maxcsize));

            /* compute leading ndrow1-by-ndrow1 lower
             * triangular block of C, C1 = L1*L1' */
            ...
            if (useGPU) {
              ...
              if (GPUavailable == 1) {
                if (ndrow2 * L_ENTRY >= CHOLMOD_ND_ROW_LIMIT &&
                   ...
                   * GPU-eligible descendants flag to
...
#endif
#ifdef REAL
              BLAS_dsyrk("L", "N", ndrow1, ndcol,    /* N, K: L1 is
                                                        ndrow1-by-ndcol*/
                         one,                        /* ALPHA:  1 */
                         Lx + L_ENTRY * pdx1, ndrow, /* A, LDA: L1, ndrow */
                         zero,                       /* BETA:   0 */
                         C, ndrow2);                 /* C, LDC: C1 */
#else
              BLAS_zherk("L", "N", ndrow1, ndcol,    /* N, K: L1 is
                                                        ndrow1-by-ndcol*/
                         one,                        /* ALPHA:  1 */
                         Lx + L_ENTRY * pdx1, ndrow, /* A, LDA: L1, ndrow */
                         zero,                       /* BETA:   0 */
...
#endif
              /* compute remaining
               * (ndrow2-ndrow1)-by-ndrow1 block of C,
               * C2 = L2*L1' */
              if (ndrow3 > 0) {
#ifndef NTIMER
//                Common->CHOLMOD_CPU_GEMM_CALLS++;
...
//                Common->CHOLMOD_CPU_GEMM_TIME += SuiteSparse_time() - tstart;
              ...
              /* ----------------------------------------------------------
               */
              /* construct relative map to assemble d
               * into s */
              /* ----------------------------------------------------------
               */
              ...
              /* ----------------------------------------------------------
               */
              /* assemble C into supernode s using the
               * relative map */
              /* ----------------------------------------------------------
               */
...
#pragma omp critical
              ...
              supernodeUsedGPU = 1; /* GPU was used for this
                                       supernode*/
              Common->ibuffer++;    /* gpu_updateC is
                                     * asynchronous, so
                                     * use the next host
                                     * buffer for the
                                     * next supernode */
            ...
            /* --------------------------------------------------------------
             */
            /* prepare this supernode d for its next
             * ancestor */
            /* --------------------------------------------------------------
             */
...
#pragma omp critical
              {
                /* If node s is being repeated, Head
                 * [dancestor] has already been
                 * cleared (set to EMPTY).  It must
                 * remain EMPTY.  The dancestor will
                 * not be factorized since the
                 * factorization terminates at node s.
                 */
                Lpos[d] = pdi2 - pdi;
                if (Lpos[d] < ndrow) {
                  dancestor = SuperMap[Ls[pdi2]];
                  ASSERT(dancestor > s && dancestor < nsuper);
                  /* place d in the link list of its
                   * next ancestor */
                  Next[d] = Head[dancestor];
                  Head[dancestor] = d;
                }
          ...
          if (useGPU) {
            ...
            /* combine updates assembled on the GPU
             * with updates assembled on the CPU */
            TEMPLATE2(CHOLMOD(gpu_final_assembly))
            (Common, Lx, psx, nscol, nsrow, supernodeUsedGPU, &iHostBuff,
             &iDevBuff, gpu_p);
          }
          ...
          PRINT1(("\nSupernode with contributions A: "
                  "repeat: " ID "\n",
                  repeat_supernode));
          DEBUG(
              CHOLMOD(dump_super)(s, Super, Lpi, Ls, Lpx, Lx, L_ENTRY, Common));
          ...
          /* ------------------------------------------------------------------
           */
          /* factorize diagonal block of supernode s
           * in LL' */
          /* ------------------------------------------------------------------
           */

          /* The current supernode s is ready to
           * factorize.  It has been updated by all
           * descendant supernodes.  Let S = the
           * current supernode, which holds rows
           * k1:n-1 and columns k1:k2-1 of the updated
           * matrix.   It splits into two parts:  the
           * square diagonal block S1, and the
           * rectangular part S2.  Here, S1 is
           * factorized into L1*L1' and overwritten by
           * L1.
           *
           * If supernode s is being repeated, only
           * factorize it up to but not including the
           * column containing the problematic entry.
           */
...
#endif
          {
            /* Note that the GPU will not be used for
          ...
          /* ------------------------------------------------------------------
           */
          /* check if the matrix is not positive
           * definite */
          /* ------------------------------------------------------------------
           */

          if (repeat_supernode) {
            /* the leading part has been refactorized;
          ...
          /* info is set to one in LAPACK_*potrf if
           * blas_ok is FALSE.  It is set to zero in
           * dpotrf/zpotrf if the factorization was
           * successful.
           */
          if (CHECK_BLAS_INT && !Common->blas_ok) {
            ERROR(CHOLMOD_TOO_LARGE, "problem too large for the BLAS");
          ...
          if (info != 0) {
             ...
             * dpotrf/zpotrf do NOT report an error if
             * the diagonal of L has NaN's, only if it
             * has a zero. */
            if (Common->status == CHOLMOD_OK) {
              ERROR(CHOLMOD_NOT_POSDEF, "matrix not positive definite");
            }

            /* L->minor is the column of L that
             * contains a zero or negative diagonal
             * term. */
            L->minor = k1 + info - 1;

            /* clear the link lists of all subsequent
             * supernodes */
            for (ss = s + 1; ss < nsuper; ss++) {
              Head[ss] = EMPTY;
            }

            /* zero this supernode, and all remaining
             * supernodes */
            pend = L->xsize;
            for (p = psx; p < pend; p++) {
              /* Lx [p] = 0. ; */
            ...
            /* If L is indefinite, it still contains
             * useful information. Supernodes 0 to s-1
             * are valid, similar to MATLAB
             * [R,p]=chol(A), where the 1-based p is
             * identical to the 0-based L->minor.
             * Since L->minor is in the current
             * supernode s, it and any columns to the
             * left of it in supernode s are also all
             * zero.  This differs from [R,p]=chol(A),
             * which contains nonzero rows 1 to p-1.
             * Fix this by setting repeat_supernode to
             * TRUE, and repeating supernode s.
             *
             * If Common->quick_return_if_not_posdef
             * is true, then the entire supernode s is
             * not factorized; it is left as all zero.
             */

            if (info == 1 || Common->quick_return_if_not_posdef) {
              /* If the first column of supernode s
               * contains a zero or negative diagonal
               * entry, then it is already properly
               * set to zero.  Also, info will be 1 if
               * integer overflow occured in the BLAS.
               */
              Head[s] = EMPTY;
#ifdef GPU_BLAS
              if (useGPU) {
              ...
              /* Repeat supernode s, but only
               * factorize it up to but not including
               * the column containing the problematic
               * diagonal entry. */
              repeat_supernode = TRUE;
              s--;
              nscol_new = info - 1;
          ...
          /* ------------------------------------------------------------------
           */
          /* compute the subdiagonal block and prepare
           * supernode for its parent
           */
          /* ------------------------------------------------------------------
           */
          ...
          nsrow2 = nsrow - nscol2;
          if (nsrow2 > 0) {
            /* The current supernode is columns k1 to
             * k2-1 of L.  Let L1 be the diagonal
             * block (factorized by dpotrf/zpotrf
             * above; rows/cols k1:k2-1), and L2 be
             * rows k2:n-1 and columns k1:k2-1 of L.
             * The triangular system to solve is
             * L2*L1' = S2, where S2 is overwritten
             * with L2.  More precisely, L2 = S2 / L1'
             * in MATLAB notation.
             */
...
#endif
            {
              ...
              BLAS_dtrsm("R", "L", "C", "N", nsrow2, nscol2, /* M, N */
                         one,                                /* ALPHA: 1 */
                         Lx + L_ENTRY * psx, nsrow,     /* A, LDA: L1, nsrow */
                         Lx + L_ENTRY * (psx + nscol2), /* B, LDB, L2,
                                                           nsrow */
                         nsrow);
#else
              BLAS_ztrsm("R", "L", "C", "N", nsrow2, nscol2, /* M, N */
          ...
          Head[s] = EMPTY; /* link list for supernode
                              s no longer needed */
          end_marked[s] = 1;
          /* clear the Map (debugging only, to detect
           * changes in pattern of A)
           */
          DEBUG(for (k = 0; k < nsrow; k++) Map[Ls[psi + k]] = EMPTY);
          DEBUG(
              CHOLMOD(dump_super)(s, Super, Lpi, Ls, Lpx, Lx, L_ENTRY, Common));
          ...
          if (repeat_supernode) {
            /* matrix is not positive definite;
             * finished clean-up for supernode
             * containing negative diagonal */
