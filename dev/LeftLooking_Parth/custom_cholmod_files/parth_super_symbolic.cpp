...
static void subtree(
    ...
    Int k, Int Ap[], Int Ai[], Int Anz[], Int SuperMap[], Int Sparent[],
    Int mark, Int sorted, /* true if the columns of A are sorted */
    Int k1, /* only consider A (0:k1-1,k) */

    /* input/output: */
    Int Flag[], Int Ls[], Int Lpi2[])
{
    Int p, pend, i, si;
    p = Ap[j];
    ...
    cholmod_common* Common)
{
    ...
    Int *Wi, *Wj, *Super, *Snz, *Ap, *Ai, *Flag, *Head, *Ls, *Lpi, *Lpx, *Fnz,
        *Sparent, *Anz, *SuperMap, *Merged, *Nscol, *Zeros, *Fp, *Fj, *ColCount,
        *Lpi2, *Lsuper, *Iwork;
    Int nsuper, d, n, j, k, s, mark, parent, p, pend, k1, k2, packed, nscol,
        nsrow, ndrow1, ndrow2, stype, ssize, xsize, sparent, plast, slast, csize,
        maxcsize, ss, nscol0, nscol1, ns, nfsuper, newzeros, totzeros, merge,
        snext, esize, maxesize, nrelax0, nrelax1, nrelax2, Asorted;
    size_t w;
    ...
    if (for_whom == CHOLMOD_ANALYZE_FOR_CHOLESKY) {
        ...
        if (Common->useGPU == 1) {
            ...
            CHOLMOD(gpu_allocate)
            (Common); // Cholesky only, not SPQR
    ...
    for (j = 1; j < n; j++) {
        ...
        if (col_to_region[j] != col_to_region[j - 1]) {
            Super[nfsuper++] = j;
            continue;
        }
        ...
        if (Parent[j - 1] != j /* parent of j-1 is not j */
            ...
            || (for_whom == CHOLMOD_ANALYZE_FOR_CHOLESKY && L->useGPU && (j - Super[nfsuper - 1] + 1) * ColCount[Super[nfsuper - 1]] * sizeof(double) * 2 >= Common->devBuffSize)
#endif
        ) {
    ...
    for (s = nfsuper - 2; s >= 0; s--) {
        ...
        if (ns <= nrelax0) {
            ...
            if (xnewzeros == 0) {
                ...
                merge = ((ns <= nrelax1 && z < zrelax0) || (ns <= nrelax2 && z < zrelax1) || (z < zrelax2)) && (xtotsize < Int_max / sizeof(double));
            }
        }
        ...
        if (for_whom == CHOLMOD_ANALYZE_FOR_CHOLESKY && L->useGPU) {
            /* Ensure that the aggregated supernode fits in the device
               supernode buffers */
            double xns = (double)ns;
            if (((xns * xns) + xns * (lnz1 - nscol1)) * sizeof(double) * 2 >= Common->devBuffSize) {
                merge = FALSE;
            }
        }
#endif
    ...
    this->super_parents.resize(nsuper); // PARTH IS ADDED
    for (s = 0; s < nsuper; s++) {
        j = Super[s + 1] - 1; /* last node in supernode s */
        parent = Parent[j]; /* parent of last node */
        Sparent[s] = (parent == EMPTY) ? EMPTY : SuperMap[parent];
        super_parents[s] = Sparent[s];
    ...
    xsize = 0;
    xxsize = 0;
    find_xsize = for_whom == CHOLMOD_ANALYZE_FOR_CHOLESKY || for_whom == CHOLMOD_ANALYZE_FOR_SPQRGPU;
    for (s = 0; s < nsuper; s++) {
    ...
    (CHOLMOD_PATTERN, TRUE, TRUE, TRUE, TRUE, L, Common);
    ...
    if (for_whom == CHOLMOD_ANALYZE_FOR_CHOLESKY || for_whom == CHOLMOD_ANALYZE_FOR_SPQRGPU) {
        Lpx[0] = 0;
        p = 0;
        for (s = 0; s < nsuper; s++) {
    ...
    for (s = 0; s < nsuper; s++) {
        ...
        for (k = k1; k < k2; k++) {
            ...
            if (stype != 0) {
                subtree(k, k, Ap, Ai, Anz, SuperMap, Sparent, mark, Asorted, k1, Flag,
                    Ls, Lpi2);
            }
            else {
                ...
                for (; p < pend; p++) {
                    subtree(Fj[p], k, Ap, Ai, Anz, SuperMap, Sparent, mark, Asorted, k1,
                        Flag, Ls, Lpi2);
                }
            }
        }
    ...
    for (s = 0; s < nsuper; s++) {
        ...
        CHOLMOD(dump_super)
        (s, Super, Lpi, Ls, NULL, NULL, 0, Common);
    }
#endif
    ...
    /* maxcsize could be determined before L->s is allocated and defined,
     *which would mean that all memory requirements for both the symbolic and
     *numeric factorizations could be computed using O(nnz(A)+O(n)) space.
     *However, it would require a lot of extra work.  The analysis phase,
     *above, would need to be duplicated, but with Ls not kept; instead, the
     *algorithm would keep track of the current s and slast for each supernode
     *d, and update them when a new row index appears in supernode d.  An
     *alternative would be to do this computation only if the allocation of
     *L->s failed, in which case the following code would be skipped.
     *
     ...
     * in the figure below correspond to the columns of a subsequent
     *supernode, and the dots are the entries in that ancestore.
     *
     *	    c
     *	    c c
     *	    c c c
...
/* Analyzes A, AA', or A(:,f)*A(:,f)' in preparation for a supernodal
 * numeric factorization.  The user need not call this directly;
 * cholmod_analyze is a "simple" wrapper for this routine.
 *
 * This function does all the analysis for a supernodal Cholesky
 * factorization.
 *
...
int ParthSolverAPI::cholmod_super_symbolic_custom(
    ...
    cholmod_common* Common)
{
    return cholmod_super_symbolic2_custom(CHOLMOD_ANALYZE_FOR_CHOLESKY, A, F,
        Parent, L, Common);
}
} // namespace PARTH
