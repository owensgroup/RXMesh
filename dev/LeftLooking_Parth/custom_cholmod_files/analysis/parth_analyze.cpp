...
#include "cholmod.h"  // Include first - has its own extern "C" guards and includes CUDA headers
extern "C" {
#include "cholmod_internal.h"  // Internal header without extern "C" guards, but cholmod.h already included
}
...
cholmod_factor *ParthSolverAPI::cholmod_analyze_p_custom // returns symbolic factor L
(
    // input:
    cholmod_sparse *A,  // matrix to order and analyze
...
int ParthSolverAPI::cholmod_analyze_ordering_custom
(
    // input:
...
cholmod_factor *ParthSolverAPI::cholmod_analyze_p2_custom
(
    // input:
...
)
{
    ...
    Lparent  = (int32_t*) CHOLMOD(malloc) (n, sizeof (Int), Common) ;
    Perm     = (int32_t*) CHOLMOD(malloc) (n, sizeof (Int), Common) ;
    ColCount = (int32_t*) CHOLMOD(malloc) (n, sizeof (Int), Common) ;
    if (Common->status < CHOLMOD_OK)
    {
        // out of memory
        FREE_WORKSPACE_AND_RETURN ;
    }
    Lperm = (int32_t*) L->Perm ;
    Lcolcount = (int32_t*) L->ColCount ;
    Common->anz = EMPTY ;
    ...
    for (method = 0 ; method <= nmethods ; method++)
    {
        ...
        if (!skip_analysis)
        {
            if (!cholmod_analyze_ordering_custom (A, ordering, Perm, fset, fsize,
    ...
    if (skip_best)
    {
        if (!cholmod_analyze_ordering_custom (A, L->ordering, Lperm, fset, fsize,
