...
/* Permute and transpose a matrix.  Allocates the A1 and A2 matrices, if
 * needed, or returns them as NULL if not needed.
 */
...
static int permute_matrices(
    ...
    cholmod_sparse **A2_handle, cholmod_sparse **S_handle,
...
cholmod_factor *ParthSolver::cholmod_analyze_custom(
    /* ---- input ---- */
    cholmod_sparse *A, /* matrix to order and analyze */
    /* --------------- */
    cholmod_common *Common) {
  return cholmod_analyze_p2_custom(TRUE, A, NULL, NULL, 0, Common);
}
...
cholmod_factor *ParthSolver::cholmod_analyze_p_custom(
    /* ---- input ---- */
    cholmod_sparse *A, /* matrix to order and analyze */
...
int ParthSolver::cholmod_analyze_ordering_custom(
    /* ---- input ---- */
  ...
  ok = permute_matrices(A, ordering, Perm, fset, fsize, do_rowcolcounts, &A1,
                        &A2, &S, &F, Common);
  ...
  Common->status =
      (!ok && Common->status == CHOLMOD_OK) ? CHOLMOD_INVALID : Common->status;
  ...
  if (do_rowcolcounts) {
    ok =
        ok && CHOLMOD(rowcolcounts)(A->stype ? F : S, fset, fsize, Parent, Post,
...
#define FREE_WORKSPACE_AND_RETURN                                              \
  {                                                                            \
    ...
    CHOLMOD(free)                                                              \
    (n, sizeof(Int), Lparent, Common);                                         \
    CHOLMOD(free)                                                              \
    (n, sizeof(Int), Perm, Common);                                            \
    CHOLMOD(free)                                                              \
    (n, sizeof(Int), ColCount, Common);                                        \
...
cholmod_factor *ParthSolver::cholmod_analyze_p2_custom(
    /* ---- input ---- */
  ...
  if (default_strategy) {
    ...
    amd_backup =
        (nmethods > 1) ||
        (nmethods == 1 && (Common->method[0].ordering == CHOLMOD_METIS ||
  ...
  for (method = 0; method <= nmethods; method++) {
    ...
    if (ordering == CHOLMOD_NATURAL) {
      ...
      if (UserPerm == NULL) {
        ...
        if (this->Options().getVerbose()) {
          std::cout << "*** PARTH: skip, no user perm given\n";
        }
        continue;
      }
      if (this->Options().getVerbose()) {
        std::cout << "*** PARTH: Performing user perm \n";
      }
      for (k = 0; k < n; k++) {
      ...
      /* -------------------------------------------------------------- */
      if (this->Options().getVerbose()) {
        std::cout << "Performing AMD perm \n";
      }
      amd_backup = FALSE; /* no need to try AMD twice ... */
      CHOLMOD(amd)
      (A, fset, fsize, Perm, Common);
      skip_analysis = TRUE;
      ...
      if (A->stype) {
        ...
        */
        ...
        CHOLMOD(colamd)
        (A, fset, fsize, FALSE, Perm, Common);
      }
    } else if (ordering == CHOLMOD_METIS) {
      if (this->Options().getVerbose()) {
        std::cout << "Performing METIS_NodeND perm \n";
      }
      ...
      if (this->Options().getVerbose()) {
        std::cout << "Performing METIS_ComputeVertexSeparator perm \n";
      }
      /* -------------------------------------------------------------- */
    ...
    if (!skip_analysis) {
      if (!CHOLMOD(analyze_ordering)(A, ordering, Perm, fset, fsize, Parent,
                                     Post, ColCount, First, Level, Common)) {
    ...
    if (default_strategy && ordering == CHOLMOD_AMD) {
      if ((Common->fl < 500 * Common->lnz) || (Common->lnz < 5 * Common->anz)) {
  ...
  if (skip_best) {
    if (!CHOLMOD(analyze_ordering)(A, L->ordering, Lperm, fset, fsize, Lparent,
                                   Post, Lcolcount, First, Level, Common)) {
      /* out of memory, or method failed */
      FREE_WORKSPACE_AND_RETURN;
    }
  }
  ...
  if (Common->supernodal > CHOLMOD_AUTO ||
      (Common->supernodal == CHOLMOD_AUTO && Common->lnz > 0 &&
       (Common->fl / Common->lnz) >= Common->supernodal_switch)) {
    cholmod_sparse *S, *F, *A2, *A1;

    permute_matrices(A, L->ordering, Lperm, fset, fsize, TRUE, &A1, &A2, &S, &F,
                     Common);

    // PARTH SOLVER CHANGE - Compute the first nonzero of each column
    computeFirstchild(S->ncol, (int *)S->p, (int *)S->i);
    /* workspace: Flag (nrow), Head (nrow), Iwork (5*nrow) */
    //        CHOLMOD(super_symbolic2)
    cholmod_super_symbolic2_custom(for_whom, S, F, Lparent, L, Common);
    PRINT1(("status %d\n", Common->status));
