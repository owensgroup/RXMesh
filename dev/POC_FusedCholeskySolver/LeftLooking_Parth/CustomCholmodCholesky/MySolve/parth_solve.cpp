...
)
{
    ...
    Yx = (double*)Y->x;
    Yz = (double*)Y->z;
    ASSERT(((Int)Y->nzmax) >= nrow * nk * ((X->xtype != CHOLMOD_REAL && Y->xtype == CHOLMOD_REAL) ? 2 : 1));
    ...
    cholmod_common* Common)
{
    ...
    ok = CHOLMOD(solve2_custom)(sys, L, B, NULL, &X, NULL, &Y, &E, Common);
        ...
        cholmod_common* Common)
{
    ...
    if ((sys == CHOLMOD_P || sys == CHOLMOD_Pt || sys == CHOLMOD_A) && L->ordering != CHOLMOD_NATURAL) {
    ...
    if (Bset) {
        ...
        Xset = *Xset_Handle;
        if (Xset == NULL || (Int)Xset->nrow != n || (Int)Xset->ncol != 1 || (Int)Xset->nzmax < n || Xset->itype != CHOLMOD_PATTERN) {
            /* this is done only once, for the 1st call to cholmod_solve */
            CHOLMOD(free_sparse)
            (Xset_Handle, Common);
            Xset = CHOLMOD(allocate_sparse)(n, 1, n, FALSE, TRUE, 0, CHOLMOD_PATTERN,
                Common);
            *Xset_Handle = Xset;
        }
        Xset->sorted = FALSE;
        ...
        if (sys == CHOLMOD_A || sys == CHOLMOD_LDLt) {
            cholmod_super_lsolve_custom(L, Y, E, Common); /* Y = L\Y */
            cholmod_super_ltsolve_custom(L, Y, E, Common); /* Y = L'\Y*/
        }
