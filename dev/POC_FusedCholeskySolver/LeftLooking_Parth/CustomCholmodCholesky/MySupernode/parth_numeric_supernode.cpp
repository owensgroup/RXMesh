    ...
    cholmod_common* Common)
{
    ...
    if (L->xtype != CHOLMOD_PATTERN) {
        if (!((A->xtype == CHOLMOD_REAL && L->xtype == CHOLMOD_REAL) || (A->xtype == CHOLMOD_COMPLEX && L->xtype == CHOLMOD_COMPLEX) || (A->xtype == CHOLMOD_ZOMPLEX && L->xtype == CHOLMOD_COMPLEX))) {
    ...
    if (symbolic) {
        ...
        ((A->xtype == CHOLMOD_REAL) ? CHOLMOD_REAL : CHOLMOD_COMPLEX, TRUE, TRUE,
            TRUE, TRUE, L, Common);
        if (Common->status < CHOLMOD_OK) {
    ...
    if (Options().getNumericReuseType() == NumericReuseType::NUMERIC_NO_REUSE_PARALLEL) {
        ...
        case CHOLMOD_COMPLEX:
            ok = c_parth_super_numeric_parallel(A, F, beta, L, C, Common, *this);
            break;
