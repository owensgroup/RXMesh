...
#define LAPACK_DSPTRF dsptrf_64_
...
void BLAS_DGEMV(char* trans, BLAS_INT* m, BLAS_INT* n, double* alpha, double* A,
    BLAS_INT* lda, double* X, BLAS_INT* incx, double* beta,
...
#define BLAS_dgemv(trans, m, n, alpha, A, lda, X, incx, beta, Y, incy)                                       \
    {                                                                                                        \
        ...
        if (CHECK_BLAS_INT && !(EQ(M, m) && EQ(N, n) && EQ(LDA, lda) && EQ(INCX, incx) && EQ(INCY, incy))) { \
            BLAS_OK = FALSE;                                                                                 \
        }                                                                                                    \
...
void BLAS_ZGEMV(char* trans, BLAS_INT* m, BLAS_INT* n, double* alpha, double* A,
    BLAS_INT* lda, double* X, BLAS_INT* incx, double* beta,
    double* Y, BLAS_INT* incy);
...
#define BLAS_dtrsm(side, uplo, transa, diag, m, n, alpha, A, lda, B, ldb)                \
    {                                                                                    \
        ...
        if (CHECK_BLAS_INT && !(EQ(M, m) && EQ(N, n) && EQ(LDA, lda) && EQ(LDB, ldb))) { \
            BLAS_OK = FALSE;                                                             \
        }                                                                                \
        if (!CHECK_BLAS_INT || BLAS_OK) {                                                \
...
#define BLAS_dgemm(transa, transb, m, n, k, alpha, A, lda, B, ldb, beta, C,                                          \
    ...
    {                                                                                                                \
        ...
        if (CHECK_BLAS_INT && !(EQ(M, m) && EQ(N, n) && EQ(K, k) && EQ(LDA, lda) && EQ(LDB, ldb) && EQ(LDC, ldc))) { \
            BLAS_OK = FALSE;                                                                                         \
        }                                                                                                            \
        if (!CHECK_BLAS_INT || BLAS_OK) {                                                                            \
...
#define BLAS_zgemm(transa, transb, m, n, k, alpha, A, lda, B, ldb, beta, C,                                          \
    ldc)                                                                                                             \
    {                                                                                                                \
        BLAS_INT M = m, N = n, K = k, LDA = lda, LDB = ldb, LDC = ldc;                                               \
        if (CHECK_BLAS_INT && !(EQ(M, m) && EQ(N, n) && EQ(K, k) && EQ(LDA, lda) && EQ(LDB, ldb) && EQ(LDC, ldc))) { \
...
#define LAPACK_dsptrf(uplo, n, A, IPIV, lda, info)           \
    {                                                        \
        BLAS_INT N = n, LDA = lda, INFO = 1;                 \
        if (CHECK_BLAS_INT && !(EQ(N, n) && EQ(LDA, lda))) { \
            BLAS_OK = FALSE;                                 \
        ...
        if (!CHECK_BLAS_INT || BLAS_OK) {                    \
            LAPACK_DSPTRF(uplo, &N, A, IPIV, &LDA, &INFO);   \
        }                                                    \
        info = INFO;                                         \
    }
...
void BLAS_DGER(BLAS_INT* m, BLAS_INT* n, double* alpha, double* X,
    BLAS_INT* incx, double* Y, BLAS_INT* incy, double* A,
    BLAS_INT* lda);
...
void BLAS_ZGER(BLAS_INT* m, BLAS_INT* n, double* alpha, double* X,
    BLAS_INT* incx, double* Y, BLAS_INT* incy, double* A,
    BLAS_INT* lda);
