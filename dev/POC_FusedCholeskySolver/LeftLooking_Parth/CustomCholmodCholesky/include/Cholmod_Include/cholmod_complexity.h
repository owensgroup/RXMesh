...
#define Z_DIV(x, z, p, ax, az, q) \
    SuiteSparse_config.divcomplex_func(x[p], z[p], ax[q], az[q], &x[p], &z[p])
...
#define RETURN_IF_XTYPE_INVALID(A, xtype1, xtype2, result)                                                                                                                  \
    {                                                                                                                                                                       \
        if ((A)->xtype < (xtype1) || (A)->xtype > (xtype2) || ((A)->xtype != CHOLMOD_PATTERN && ((A)->x) == NULL) || ((A)->xtype == CHOLMOD_ZOMPLEX && ((A)->z) == NULL)) { \
