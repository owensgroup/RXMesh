...
static int TEMPLATE(parth_super_numeric_reuse)(
    /* ---- input ---- */
...
#pragma omp parallel for num_threads(CHOLMOD_OMP_NUM_THREADS) if (n > 128) \
    ...
    if (solver.chol_L_prev != nullptr && solver.Options().getNumericReuseType() == PARTH::ParthSolver::NUMERIC_REUSE_BASE && solver.copy_prev_factor) {
...
#pragma omp parallel for default(none) private(k1, k2, nscol, psi, psx, psend, \
    ...
    //  std::vector<std::vector<int>> neighbors(nsuper, std::vector<int>());
    //  bool redo_supernode_without_cached_nodes = false;
    /* ---------------------------------------------------------------------- */
    /* supernodal numerical factorization */
    /* ---------------------------------------------------------------------- */
    //  solver.no_spd_supernode_ids.clear();
    //  solver.no_spd_supernode_ids.resize(nsuper, 0);
    for (s = 0; s < nsuper; s++) {
        ...
        PRINT1(("====================================================\n"
                "S " ID " k1 " ID " k2 " ID " nsrow " ID " nscol " ID " psi " ID
                " psend "
                "" ID " psx " ID "\n",
...
#pragma omp parallel for num_threads(CHOLMOD_OMP_NUM_THREADS) if (nsrow > 128)
        ...
        if (useGPU) {
            ...
            (Common, Super, &s, Lpi, Lpos, Head, Next, Previous, &ndescendants, &tail,
                &mapCreatedOnGpu, gpu_p);
        }
#endif
...
#pragma omp parallel for private(p, pend, pfend, pf, i, j, imap, q) \
    num_threads(CHOLMOD_OMP_NUM_THREADS) if (k2 - k1 > 64)
                    ...
                    for (; p < pend; p++) {
                        ...
                        if (i >= k) {
                            ...
                            if (imap >= 0 && imap < nsrow) {
                                ...
                                L_MULTADD(Lx, (imap + (psx + (k - k1) * nsrow)), Ax, Az, p,
                                    fjk);
                            }
                        }
                    }
        ...
        DEBUG(CHOLMOD(dump_super)(s, Super, Lpi, Ls, Lpx, Lx, L_ENTRY, Common));
        PRINT1(("\n\n"));
        ...
        if (!repeat_supernode) {
            ...
            //      if (!redo_supernode_without_cached_nodes) {
        ...
        for (d = Head[s]; d != EMPTY; d = Next[d]) {
            ...
            DEBUG(CHOLMOD(dump_super)(d, Super, Lpi, Ls, Lpx, Lx, L_ENTRY, Common));
        }
...
#endif
        {
            ...
            //      if (redo_supernode_without_cached_nodes) {
            //        assert(neighbors[s][cnt++] == d);
            //      } else {
            //        neighbors[s].emplace_back(d);
            //      }

            //      if (solver.super_is_cached[d] == 1) {
            //        dnext = Next[d];
            //        continue;
            //      }
            ...
            //      if (solver.super_is_cached[d] == 1 &&
            //          redo_supernode_without_cached_nodes && Ls[pdi2] > k1 + info -
            //          1) {
            //        dnext = Next_save[d];
            //        continue;
            //      }
...
#endif
            {
...
#endif
#ifdef REAL
                BLAS_dsyrk("L", "N", ndrow1, ndcol, /* N, K: L1 is ndrow1-by-ndcol*/
                    one, /* ALPHA:  1 */
                ...
                DEBUG(
                    CHOLMOD(dump_real)("C", C, ndrow2, ndrow1, TRUE, L_ENTRY, Common));
...
#pragma omp parallel for private(j, i, px, q) \
    num_threads(CHOLMOD_OMP_NUM_THREADS) if (ndrow1 > 64)
        ...
        if (useGPU) {
            ...
            (Common, Lx, psx, nscol, nsrow, supernodeUsedGPU, &iHostBuff, &iDevBuff,
                gpu_p);
        }
#endif
        ...
        //    nscol2 = (repeat_supernode) ? (nscol_new) : (nscol);//TODO:CHECK THIS
        nscol2 = (repeat_supernode) ? (nscol_new) : (nscol);
...
#endif
        {
            ...
            //      if (redo_supernode_without_cached_nodes) {
            //        if (info != 0) {
            //          std::cout << "||||||||||||||||| There is no hope for this
            //          supernode "
            //                       "- redo the whole "
            //                       "factorization"
            //                    << std::endl;
            //          std::fill(solver.super_is_cached.begin(),
            //                    solver.super_is_cached.end(), 0);
            //          redo_supernode_without_cached_nodes = false;
            //          s = -1;
            //          // Reset head and next
            //          for (int sup = 0; sup < nsuper; sup++) {
            //            Head[sup] = reset_head[sup];
            //            Lpos[sup] = reset_Lpos[sup];
            //            Next[sup] = reset_next[sup];
            //            neighbors[sup].clear();
            //          }
            //          continue;
            //        }
            ...
            //        redo_supernode_without_cached_nodes = false;
            //      } else {
            //        redo_supernode_without_cached_nodes = true;
            //        std::cout << "||||||||||||||||| X IS: " << s << std::endl;
            //        solver.no_spd_supernode_ids[s] = 1;
            //        s--;
            //        nscol_new = nscol; // It is different from normal approach
            //        because
