...
static void
TEMPLATE(ldl_dsolve)(cholmod_factor* L,
    ...
    Int* Yseti, Int ysetlen)
{
    if (L->is_ll) {
        ...
        if (sys == CHOLMOD_A || sys == CHOLMOD_LDLt) {
            ...
            TEMPLATE(ll_lsolve_k)
            (L, Y, Yseti, ysetlen);
            TEMPLATE(ll_ltsolve_k)
            (L, Y, Yseti, ysetlen);
        }
        ...
        if (sys == CHOLMOD_A || sys == CHOLMOD_LDLt) {
            ...
            TEMPLATE(ldl_lsolve_k)
            (L, Y, Yseti, ysetlen);
            TEMPLATE(ldl_dltsolve_k)
            (L, Y, Yseti, ysetlen);
        }
        else if (sys == CHOLMOD_LD) {
            /* Solve LDx=b */
            TEMPLATE(ldl_ldsolve_k)
            (L, Y, Yseti, ysetlen);
        }
