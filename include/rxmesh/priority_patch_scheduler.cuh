...
struct PriorityPatchScheduler
{

    __device__ __host__ PriorityPatchScheduler()
        : list(nullptr),
          priority(nullptr),
          count(nullptr),
    ...
    __device__ __host__
    PriorityPatchScheduler(const PriorityPatchScheduler& other) = default;
    __device__ __host__ PriorityPatchScheduler(PriorityPatchScheduler&&) =
        default;
    __device__ __host__ PriorityPatchScheduler& operator=(
        const PriorityPatchScheduler&) = default;
    __device__ __host__ PriorityPatchScheduler& operator=(
        PriorityPatchScheduler&&)                 = default;
    __device__ __host__ ~PriorityPatchScheduler() = default;
     ...
     * @brief
     */
    __host__ void sort(cudaStream_t stream = NULL)
    {
        LessThan less_than;
    ...
    __host__ void print_list() const
    {
        ...
        std::vector<uint32_t> h_priority(capacity);
        CUDA_ERROR(cudaMemcpy(h_list.data(),
                              list,
                              h_list.size() * sizeof(uint32_t),
                              cudaMemcpyDeviceToHost));
        CUDA_ERROR(cudaMemcpy(h_priority.data(),
                              priority,
                              h_priority.size() * sizeof(uint32_t),
                              cudaMemcpyDeviceToHost));
        for (uint32_t i = 0; i < h_list.size(); ++i) {
            printf(
                "\n list[%u]= %u, priority= %f", i, h_list[i], h_priority[i]);
        }
    }
