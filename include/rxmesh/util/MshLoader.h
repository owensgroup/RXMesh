...
class MshLoader
{
   public:
    ...
    {
        int tag, el_type;
        msh_struct(int _tag = 0, int _type = 0) : tag(_tag), el_type(_type)
        {
        }
        bool operator==(const msh_struct& a) const
        {
            return this->tag == a.tag && this->el_type == a.el_type;
        }
    ...
    enum
    {
        ...
        ELEMENT_PYRAMID = 7,
        // 2nd order elements
        ELEMENT_LINE_2ND_ORDER    = 8,
        ELEMENT_TRI_2ND_ORDER     = 9,
        ELEMENT_QUAD_2ND_ORDER    = 10,
        ELEMENT_TET_2ND_ORDER     = 11,
        ELEMENT_HEX_2ND_ORDER     = 12,
        ELEMENT_PRISM_2ND_ORDER   = 13,
   ...
   public:
    ...
    // create tag index
    // tag_column: ( physical (0) or elementary (1) ) specifying which tag to
    // use
   ...
   private:
    ...
    IndexVector
        m_elements;  // linear array for nodes corresponding to each element
    ...
    IntVector
        m_element_fields_components;  // Number of components for element field
