    ...
    : m_binary(binary), m_num_nodes(0), m_num_elements(0)
{
    ...
    if (!fout) {
        ...
        err_msg << "Error opening " << filename << " to write msh file."
                << std::endl;
...
void MshSaver::save_mesh(const FloatVector& nodes,
...
void MshSaver::save_nodes(const FloatVector& nodes)
{
    // Save nodes.
    // 3D hadrcoded
    ...
    if (!m_binary) {
        for (size_t i = 0; i < nodes.size(); i += 3) {
            ...
            int node_idx = i / 3 + 1;
            fout << node_idx << " " << nodes[i] << " " << nodes[i + 1] << " "
                 << nodes[i + 2] << std::endl;
        }
    } else {
        for (size_t i = 0; i < nodes.size(); i += 3) {
...
void MshSaver::save_elements(const IndexVector& elements,
                             const IntVector&   element_lengths,
                             const IntVector&   element_types,
                             const IntVector&   element_tags)
{
    ...
    if (m_num_elements > 0) {
        ...
        if (!m_binary) {
            ...
            for (size_t i = 0; i < m_num_elements; ++i) {
                ...
                fout << elem_num << " " << element_types[i] << " " << 2 << " "
                     << element_tags[i] << " " << element_tags[i] << " ";
            ...
            while (i < m_num_elements) {
                ...
                // TODO: refactor this code to be able to specify different
                // elements more effeciently
...
void MshSaver::save_scalar_field(const std::string& fieldname,
...
void MshSaver::save_vector_field(const std::string& fieldname,
                                 const FloatVector& field)
{
    ...
    if (m_binary) {
        ...
        for (size_t i = 0; i < m_num_nodes; i++) {
            int node_idx = i + 1;
            fout << node_idx << " " << field[i * 3] << " " << field[i * 3 + 1]
                 << " " << field[i * 3 + 2] << std::endl;
        }
    }
    fout << "$EndNodeData" << std::endl;
    fout.flush();
...
void MshSaver::save_elem_scalar_field(const std::string& fieldname,
                                      const FloatVector& field)
{
...
void MshSaver::save_elem_vector_field(const std::string& fieldname,
                                      const FloatVector& field)
{
    ...
    if (m_binary) {
        ...
        for (size_t i = 0; i < m_num_elements; ++i) {
            int elem_idx = i + 1;
            fout << elem_idx << " " << field[i * 3] << " " << field[i * 3 + 1]
                 << " " << field[i * 3 + 2] << std::endl;
        }
    }
...
void MshSaver::save_elem_tensor_field(const std::string& fieldname,
                                      const FloatVector& field)
{
    ...
    if (m_binary) {
        ...
        for (size_t i = 0; i < m_num_elements; i++) {
            int          elem_idx = i + 1;
            const Float* val      = &field[i * 6];
            fout << elem_idx << " " << val[0] << " " << val[5] << " " << val[4]
                 << " " << val[5] << " " << val[1] << " " << val[3] << " "
                 << val[4] << " " << val[3] << " " << val[2] << std::endl;
        }
    }
