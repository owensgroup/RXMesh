...
RXMesh::RXMesh()
    : m_num_edges(0),
      ...
      m_d_patches_info_v2(nullptr),
      m_h_patches_info_v2(nullptr),
      m_capacity_factor(0),
...
void RXMesh::build(const std::vector<std::vector<uint32_t>>& fv)
{
    ...
    m_h_patches_info_v2 =
        (PatchInfoV2*)malloc(m_num_patches * sizeof(PatchInfoV2));
    ...
    for (uint32_t p = 0; p < this->m_num_patches; ++p) {
        m_max_vertices_per_patch =
            std::max(m_max_vertices_per_patch,
                     uint32_t(m_h_patches_info_v2[p].num_vertices));

        m_max_edges_per_patch = std::max(
            m_max_edges_per_patch, uint32_t(m_h_patches_info_v2[p].num_edges));

        m_max_faces_per_patch = std::max(
            m_max_faces_per_patch, uint32_t(m_h_patches_info_v2[p].num_faces));


        m_max_not_owned_vertices = std::max(
            m_max_not_owned_vertices,
            detail::count_set_bits(m_h_patches_info_v2[p].num_vertices,
                                   m_h_patches_info_v2[p].owned_mask_v));

        m_max_not_owned_edges = std::max(
            m_max_not_owned_edges,
            detail::count_set_bits(m_h_patches_info_v2[p].num_edges,
                                   m_h_patches_info_v2[p].owned_mask_e));

        m_max_not_owned_faces = std::max(
            m_max_not_owned_faces,
            detail::count_set_bits(m_h_patches_info_v2[p].num_faces,
    ...
    const uint32_t                            patch_id)
{
    ...
    m_h_patches_info_v2[patch_id].ev =
        (LocalVertexT*)malloc(patch_num_edges * 2 * sizeof(LocalVertexT));
    m_h_patches_info_v2[patch_id].fe =
        (LocalEdgeT*)malloc(patch_num_faces * 3 * sizeof(LocalEdgeT));
    ...
    auto add_new_face = [&](const uint32_t global_face_id) {
        ...
        for (uint32_t v = 0; v < 3; ++v) {
            ...
            if (!is_added_edge[local_edge_id]) {
                ...
                m_h_patches_info_v2[patch_id].ev[local_edge_id * 2].id =
                    local_v0;
                m_h_patches_info_v2[patch_id].ev[local_edge_id * 2 + 1].id =
                    local_v1;
            }
            ...
            m_h_patches_info[patch_id].fe[local_face_id * 3 + v].id =
                local_edge_id;
            m_h_patches_info_v2[patch_id].fe[local_face_id * 3 + v].id =
                local_edge_id;
        }
    };
...
void RXMesh::build_device_v2()
{
    CUDA_ERROR(cudaMalloc((void**)&m_d_patches_info_v2,
                          m_num_patches * sizeof(PatchInfoV2)));
...
#pragma omp parallel for
    for (int p = 0; p < static_cast<int>(m_num_patches); ++p) {
        ...
        m_h_patches_info_v2[p].num_faces         = m_h_patches_ltog_f[p].size();
        m_h_patches_info_v2[p].num_edges         = m_h_patches_ltog_e[p].size();
        m_h_patches_info_v2[p].num_vertices      = m_h_patches_ltog_v[p].size();
        m_h_patches_info_v2[p].faces_capacity    = d_patch.faces_capacity;
        m_h_patches_info_v2[p].edges_capacity    = d_patch.edges_capacity;
        m_h_patches_info_v2[p].vertices_capacity = d_patch.vertices_capacity;
        m_h_patches_info_v2[p].patch_id          = p;
        m_h_patches_info_v2[p].patch_stash       = PatchStash();
        ...
        auto populate_patch_stash =
            [&](const std::vector<std::vector<uint32_t>>& ltog,
                ...
                for (uint16_t i = 0; i < num_not_owned; ++i) {
                    ...
                    m_h_patches_info_v2[p].patch_stash.insert_patch(
                        owner_patch);
        ...
        auto build_ht = [&](const std::vector<std::vector<uint32_t>>& ltog,
            ...
            for (uint16_t i = 0; i < num_not_owned; ++i) {
                ...
                if (it == ltog[owner_patch].begin() + num_owned[owner_patch]) {
                    RXMESH_ERROR(
                        "rxmesh::build_device_v2 can not find the local id of "
                        "{} in patch {}. Maybe this patch does not own "
                        "this mesh element.",
                        global_id,
                        owner_patch);
        ...
        build_ht(m_h_patches_ltog_f,
                 m_patcher->get_face_patch(),
                 m_h_num_owned_f,
                 m_h_patches_info_v2[p].patch_stash,
                 m_h_patches_info_v2[p].lp_f,
                 d_patch.lp_f);
        ...
        build_ht(m_h_patches_ltog_v,
                 ...
                 m_h_num_owned_v,
                 m_h_patches_info_v2[p].patch_stash,
                 m_h_patches_info_v2[p].lp_v,
                 d_patch.lp_v);

        CUDA_ERROR(cudaMemcpy(m_d_patches_info_v2 + p,
                              &d_patch,
                              sizeof(PatchInfoV2),
                              cudaMemcpyHostToDevice));
    }
