...
#include "rxmesh/kernels/inverse_lp_hashtable.cuh"
...
struct CavityManager2
{
    ...
    __device__ __forceinline__ uint32_t create(HandleT seed);
     ...
     */
    template <typename HandleT>
    __device__ __forceinline__ void recover(HandleT seed);
     ...
     */
    template <typename HandleT>
    __device__ __forceinline__ bool is_successful(HandleT seed);
     ...
     */
    template <typename HandleT>
    __device__ __forceinline__ HandleT get_creator(const uint16_t cavity_id);
     ...
     */
    __device__ __forceinline__ void get_vertices(const EdgeHandle eh,
                                                 VertexHandle&    v0,
                                                 VertexHandle&    v1);
     ...
     */
    __device__ __forceinline__ void get_edges(const FaceHandle fh,
                                              EdgeHandle&      e0,
                                              EdgeHandle&      e1,
                                              EdgeHandle&      e2);
    ...
    __device__ __forceinline__ bool prologue(
    ...
    __device__ __forceinline__ void for_each_cavity(
        cooperative_groups::thread_block& block,
        FillInT                           FillInFunc);
     ...
     */
    __device__ __forceinline__ int get_num_cavities() const
    {
     ...
     */
    __device__ __forceinline__ uint16_t get_cavity_size(uint16_t c) const
    {
     ...
     */
    __device__ __forceinline__ DEdgeHandle get_cavity_edge(uint16_t c,
                                                           uint16_t i) const;

    /**
     * @brief get a vertex handle to the i-th vertex in the c-th cavity
     */
    __device__ __forceinline__ VertexHandle get_cavity_vertex(uint16_t c,
                                                              uint16_t i) const;
     ...
     */
    __device__ __forceinline__ VertexHandle add_vertex();
     ...
     * attributes
     */
    __device__ __forceinline__ DEdgeHandle add_edge(const VertexHandle src,
     ...
     */
    __device__ __forceinline__ FaceHandle add_face(const DEdgeHandle e0,
     ...
     */
    __device__ __forceinline__ bool should_slice() const
    {
     ...
     */
    template <typename HandleT>
    __device__ __forceinline__ bool is_recovered(const HandleT handle)
    {
   ...
   private:
     ...
     */
    __device__ __forceinline__ void alloc_shared_memory(
        cooperative_groups::thread_block& block,
        ShmemAllocator&                   shrd_alloc);

    /**
     * @brief load the inverted hashtable into shared memory
     */
    __device__ __forceinline__ void invert_hashtable(
        cooperative_groups::thread_block& block);
    ...
    /**
     * @brief store inverted hashtable from shared memory to global memory
     */
    __device__ __forceinline__ void store_inverted_hashtable(
        cooperative_groups::thread_block& block);
     ...
     */
    __device__ __forceinline__ void construct_cavity_graph(
        cooperative_groups::thread_block& block);
     ...
     */
    __device__ __forceinline__ void calc_cavity_maximal_independent_set(
        cooperative_groups::thread_block& block);
     ...
     */
    __device__ __forceinline__ void mark_edges_through_vertices();

    /**
     * @brief propagate the cavity tag from edges to their incident vertices
     */
    __device__ __forceinline__ void mark_vertices_through_edges();


    /**
     * @brief propagate the cavity tag from face to their incident edges
     */
    __device__ __forceinline__ void mark_edges_through_faces();


    /**
     * @brief propagate the cavity tag from edges to their incident faces
     */
    __device__ __forceinline__ void mark_faces_through_edges();
     ...
     * cavity ID
     */
    __device__ __forceinline__ void mark_element_gather(
        uint16_t*      element_cavity_id,
        const uint16_t element_id,
        const uint16_t cavity_id);

    __device__ __forceinline__ void mark_element_scatter(
        uint16_t*      element_cavity_id,
        const uint16_t element_id,
        const uint16_t cavity_id);
     ...
     */
    __device__ __forceinline__ void deactivate_conflicting_cavities();

    /**
     * @brief deactivate a single cavity
     */
    __device__ __forceinline__ void deactivate_cavity(uint16_t c);

    /**
     * @brief deactivate a cavity if it requires changing ownership of elements
     * from another patch
     */
    __device__ __forceinline__ void deactivate_boundary_cavities(
        cooperative_groups::thread_block& block);
     ...
     */
    __device__ __forceinline__ void reactivate_elements();
     ...
     */
    __device__ __forceinline__ void clear_bitmask_if_in_cavity();
     ...
     */
    __device__ __forceinline__ void sort_cavities_edge_loop();
    ...
    add_element(Bitmask&       active_bitmask,
    ...
    __device__ __forceinline__ void push(const uint32_t pid);
     ...
     */
    __device__ __forceinline__ void unlock_locked_patches();

    /**
     * @brief set the dirty bit for locked patches
     */
    __device__ __forceinline__ void set_dirty_for_locked_patches();
     ...
     */
    __device__ __forceinline__ void pre_migrate(
        cooperative_groups::thread_block& block);
     ...
     * ownership
     */
    __device__ __forceinline__ void set_ownership_change_bitmask(
        cooperative_groups::thread_block& block);

    /**
     * @brief migrate vertices/edges/faces from neighbor patches to this patch
     */
    __device__ __forceinline__ bool migrate(
        cooperative_groups::thread_block& block);

    __device__ __forceinline__ bool migrate_from_patch(
        cooperative_groups::thread_block& block,
        const uint8_t                     q_stash_id,
        const uint32_t                    q);

    __device__ __forceinline__ bool soft_migrate_from_patch(
        cooperative_groups::thread_block& block,
        const uint8_t                     q_stash_id,
        const uint32_t                    q);
    ...
    add_new_patch_to_patch_stash(const uint32_t new_patch);
     ...
     */
    __device__ __forceinline__ uint16_t
    find_copy_vertex(uint16_t& local_id,
                     uint32_t& patch,
     ...
     * be mapped to their owner patch and local index in the owner patch
     */
    __device__ __forceinline__ uint16_t find_copy_edge(uint16_t& local_id,
                                                       uint32_t& patch,
                                                       uint8_t& patch_stash_id);
     ...
     * be mapped to their owner patch and local index in the owner patch
     */
    __device__ __forceinline__ uint16_t find_copy_face(uint16_t& local_id,
                                                       uint32_t& patch,
                                                       uint8_t& patch_stash_id);
     ...
     */
    __device__ __forceinline__ void change_ownership(
        cooperative_groups::thread_block& block);
     ...
     */
    __device__ __forceinline__ bool lock_new_added_patches(
        cooperative_groups::thread_block& block);
     ...
     */
    __device__ __forceinline__ bool lock_neighbour_patches(
        cooperative_groups::thread_block& block);
     ...
     * ribbonize
     */
    __device__ __forceinline__ void pre_ribbonize(
        cooperative_groups::thread_block& block);
     ...
     */
    template <typename HandleT>
    __device__ __forceinline__ bool ensure_ownership(
        cooperative_groups::thread_block& block,
        const Bitmask&                    s_ownership_change,
        const InverseLPHashTable&         s_inv_table);
     ...
     */
    __device__ __forceinline__ bool ensure_locked_patches_are_not_dirty();
     ...
     */
    __device__ __forceinline__ void recover_faces();
     ...
     */
    __device__ __forceinline__ void recover_edges();
     ...
     */
    __device__ __forceinline__ void recover_vertices();
     ...
     */
    __device__ __forceinline__ void recover_vertices_through_edges();

    /**
     * @brief set the bit for an edge in m_s_recover_e if it is incident to a
     * face that has its bit set in m_s_recover_f
     */
    __device__ __forceinline__ void recover_edges_through_faces();
     ...
     */
    __device__ __forceinline__ void recover_edges_through_vertices();

    /**
     * @brief set the bit for a face in m_s_recover_f if it is incident to an
     * edge that has its bit set in m_s_recover_e
     */
    __device__ __forceinline__ void recover_faces_through_edges();
     ...
     */
    template <typename HandleT>
    __device__ __forceinline__ std::pair<LPPair*, LPPair*> get_s_table()
    {
        if constexpr (std::is_same_v<HandleT, VertexHandle>) {
     ...
     */
    template <typename HandleT>
    __device__ __forceinline__ uint32_t get_num_elements()
    {
     ...
     */
    template <typename HandleT>
    __device__ __forceinline__ bool is_owned(uint16_t b)
    {
        if constexpr (std::is_same_v<HandleT, VertexHandle>) {
     ...
     */
    template <typename HandleT>
    __device__ __forceinline__ bool is_active(uint16_t b)
    {
        if constexpr (std::is_same_v<HandleT, VertexHandle>) {
     ...
     */
    template <typename HandleT>
    __device__ __forceinline__ bool is_in_cavity(uint16_t b)
    {
        if constexpr (std::is_same_v<HandleT, VertexHandle>) {
    ...
    template <typename HandleT>
    __device__ __forceinline__ InverseLPHashTable& get_inverse_table()
    {
        if constexpr (std::is_same_v<HandleT, VertexHandle>) {
            return m_inv_lp_v;
        }
     ...
     * global memory
     */
    __device__ __forceinline__ void verify_reading_from_global_memory(
